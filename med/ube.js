    ...
    */
      ...
      */
      ...
      $__default["default"].ubeLink = function (options) {
        var _ref = options || {},
          ...
          _ref.utmMedium;
          var utmCampaign = _ref.utmCampaign;
          _ref.utmContent;
          _ref.utmTerm;
          _ref.callback;
        ...
        isMBEPCheck(hostname);
        {
          ...
          var _utmSource = urlParams.get("utm_source");
          var _utmMedium = urlParams.get("utm_medium");
          var _utmContent = urlParams.get("utm_content");
          stageRedirect += "&utm_source=" + _utmSource + "&utm_campaign=" + _utmCampaign + "&utm_content=" + _utmContent + "&utm_medium=" + _utmMedium;
    ...
    function _authBySocial() {
      _authBySocial = _asyncToGenerator(/*#__PURE__*/_regeneratorRuntime().mark(function _callee2(_ref) {
        var formKey, socialName, successCB, errorCB, windowReference, link;
    ...
    $__default["default"].fn.ube = function (url, options) {
      ...
      function initializeFormByConfig(json) {
        ...
        var isQRForm = mbepDomains.includes(preparedDomain) && window.location.host.includes('qr');
        var isMBEPForm = mbepDomains.includes(preparedDomain) && !window.location.host.includes('qr');
        ...
        function checkHash(json, name) {
          ...
          if (query && query.hash && query.hash != "") {
            toggleLoader(true);
            ubeHostFallBack();
            var url = $__default["default"].ube.host + "/api/session/resolveHash?hash=" + query.hash + "&entity=" + name + "&source=" + window.location.hostname;
            if (redirect) {
        ...
        var handle_form = function handle_form() {
          ...
          function ajaxSubmitFormInternal(data, isSearchOnly) {
            ...
            var useCoreCRM = options.useCoreCRM && requestPath.includes('/api/session/core/registration/');
            ...
            $__default["default"].ajax({
              ...
              error: function error(xhr, resp, text) {
                ...
                if (/-reg$/.test(path)) {
                  ...
                  if (isP1Form) gaPush('dl_auth_phone', 'fail - phone', 'technical');else gaPush(_category5, "form - phone_number - reason: technical");
                } else if (/-phone-send$/.test(path)) {
                  action_path = "/esb/" + path + "/submission";
              ...
              function submissionSuccess(result) {
                ...
                if (name.includes('phone-send')) {
                  ...
                  var action = "form - phone - (".concat(result.event === 'loginSuccess' ? 'in_base' : 'not_in_base', ")");
          ...
          container.off("click", ".ube-trigger-event").on("click", ".ube-trigger-event", function (e) {
            ...
            var formData = getFormData();
            if (event == "resendCode" || event == "resendPhoneCode" || event == "resendEmailCode") $__default["default"].ajax({
              ...
              success: function success(t) {
                if (t.message) showPopup(t.message);else if (t.error) showPopup(t.error);else {
