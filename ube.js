var library = (function (exports, $$1) {
    ...
    */
    ...
    function extractErrorMessage$1(errorsDetail, errorMessage) {
      var _errorsDetail$;
            ...
            onFormLoad: function onFormLoad() {
              ...
              if (ubeSession('shouldSkipEmail') === 'false') {
                ...
                $('.skipEmailOptin').on('click', function () {
                  ...
                  $('input[name="skipEmailOptin"]').val(skipEmailOptinValue);
                ...
                $('input[name="skipEmailOptin"]').val(false);
                $('.ube-registration-switch').hide();
                $('.ube-registration-block').show();
              }
              options.onFormLoad && options.onFormLoad(key);
    ...
    $__default["default"].fn.ube = function (url, options) {
      ...
      function initializeFormByConfig(json) {
        ...
        var validationState = {};
        var isQRForm = isQRDomain();
        var isMBEPForm = isMbepDomain() && !window.location.host.includes('qr');
        var categoryPrefix = getCategoryPrefix();
        var remainingAttemptsCount, phoneConfirmationType;
        ...
        var handle_form = function handle_form() {
          ...
          function ajaxSubmitFormInternal(data, isSearchOnly) {
            var _options, _payload, _payload2;
            ...
            if (((_payload = payload) === null || _payload === void 0 || (_payload = _payload.data) === null || _payload === void 0 ? void 0 : _payload.skipEmailOptin) === 'true') {
              payload.data.skipEmailOptin = true;
            } else if (((_payload2 = payload) === null || _payload2 === void 0 || (_payload2 = _payload2.data) === null || _payload2 === void 0 ? void 0 : _payload2.skipEmailOptin) === 'false') {
          ...
          function initializeFaceCapture() {
            ...
            var faceState = {
              ...
              tryCount: 1,
              options: options,
              primaryFaceVendor: ((_options$config3 = options.config) === null || _options$config3 === void 0 || (_options$config3 = _options$config3.av) === null || _options$config3 === void 0 || (_options$config3 = _options$config3.face) === null || _options$config3 === void 0 ? void 0 : _options$config3.primaryFaceVendor) || 'idx',
              onSuccess: onSuccess,
              onError: onError
            };
            if (faceState.primaryFaceVendor === 'yoti' && (renderTarget.data("init") || !token || !tokenUrl)) return false;
            ...
            function sendBlobToServer() {
              toggleLoader(true);
              runWithPriority(faceState.primaryFaceVendor, vendorsList, faceState);
            }
            function onSuccess(data) {
              if (faceState.primaryFaceVendor === 'idx') {
                setFieldValue("tokenUrl", "");
                setFieldValue("token", data.token);
              } else if (faceState.primaryFaceVendor === 'yoti') {
                setFieldValue("token", data.token);
              }
            ...
            //             console.error("Error from ACS API");
            //             console.error(err);
            //             logACSResultToELK(eventType, false, true, 'Network Error', imageData, b64Img, name, true);
            //             form.trigger("submitNoValidation");
            //         },
            ...
            //                     logACSResultToELK(eventType, true, false, null, imageData, b64Img, name, true, token);
            //                     gaPush(categoryPrefix + "av_fr_success", `form - fr_${type} - ${label}`);
            //
            //                     if (callback) callback();
            //                 }
            //                 else if (data.next_validation_type === "DOCUMENT" || window.bad) {
            //                     let label;
            //                     if (data.errorMessage === "Technical Error") label = 'technical'
            ...
            //                     }
            //
            //                     gaPush(categoryPrefix + "av_fr_fail", `form - fr_${type} - ${label} - reason: ${detailMessage}`);
            //
            //                     logACSResultToELK(eventType, false, false, detailMessage, imageData, b64Img, name, true);
            //                     if (data.errors_detail) {
            ...
            //                         else if (tryCount === 1) label = 'first'
            //                         else label = 'second';
            //                         logACSResultToELK(eventType, false, false, data.errors_detail[0].code || "Technical Error", imageData, b64Img, name, true);
            //                     }
            //                     if($(".ube-face-error").length > 0) {
            //                         $(".ube-face-container").hide();
            //                         $(".ube-face-error").fadeIn(200);
            //                         if (callback) callback();
            //                     } else if ($(".ube-visibility-show-for-documentMethod").length > 0 &&
            //                         $(".ube-visibility-show-for-megafonMethod").length === 0) {
            ...
            //                 else if (data.error_message) {
            //                     let label;
            //                     if (!data.error_message) label = 'technical'
            //                     else if (tryCount === 1) label = 'first'
            //                     else label = 'second';
            //                     console.log(data.error_message);
            //                     gaPush(categoryPrefix + "av_fr_fail", `form - fr_${type} - ${label} - reason: ${data.error_message}`);
            //
            ...
            //
            //                         const title = faceErrors[code] ? faceErrors[code].title : faceErrors.default.title;
            //                         const subtitle = faceErrors[code] ? faceErrors[code].subtitle : faceErrors.default.subtitle;
            //                         const text = faceErrors[code] ? faceErrors[code].text : faceErrors.default.text;
            //
    ...
    exports.debug = debug;
    exports.extractErrorMessage = extractErrorMessage$1;
