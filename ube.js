var library = (function (exports, $$1) {
    ...
    function logACSResultToELK(type, result, error, errorText, photoData, image, formKey, priorityFunction, token, faceId) {
      ubeHostFallBack();
      var data = {
      ...
      $__default["default"].ajax({
        ...
        success: function success(data) {
          if (data.jwt && priorityFunction) ubeCookie$1(cookies.UBE_JWT_FACE_TOKEN, data.jwt);
    ...
    */
    ...
    var getParameters = function getParameters() {
      ...
      if (parameters.length === 1) return searchParams.get(parameters[0]);
      return parameters.map(function (parameter) {
        return searchParams.get(parameter);
      });
    };
    var formKeyToConfigKey = function formKeyToConfigKey(key) {
      var keyVariants = ['optin', 'av', 'face', 'login', 'remind', 'cabinet', 'vk', 'terms', 'phone-send', 'phone-check', 'phone', 'coupon', 'email-send', 'email-check'];
      var regexPart = keyVariants.join('|');
      var regex = new RegExp("(".concat(regexPart, ")$"));
      return key.replace(regex, 'reg');
    };
    var getKeyFromUrl = function getKeyFromUrl(str) {
      var match = str === null || str === void 0 ? void 0 : str.match(/ube2-[^\/]+/);
      return match ? formKeyToConfigKey(match[0]) : str;
    };
    ...
    var isP1Domain = function isP1Domain() {
      return window.location.host.includes('premium.one');
    };
    var isQRDomain = function isQRDomain() {
      return isMbepDomain() && window.location.host.includes('qr');
    };
    ...
    function gaPush$1(category, action, label) {
    ...
    function gaUserId(userId) {
      try {
        ...
        if (ym && ymCounterId && !hasSentUserParams) {
          ym(ymCounterId, 'userParams', {
            UserID: userId
          });
          hasSentUserParams = true;
        }
      } catch (e) {}
      try {
    ...
    var getCategoryPrefix = function getCategoryPrefix() {
      return isQRDomain() ? 'dl_qr_' : 'dl_';
    };
    var updatePhoneTypeActions = function updatePhoneTypeActions(actions, suffix) {
      for (var key in actions) {
        actions[key] += suffix;
      }
      return actions;
    };
    var gaSkipEmailOptin = function gaSkipEmailOptin(category, action) {
      if (ubeSession('shouldSkipEmail') === 'true') {
        gaPush$1(category, "".concat(action, " - with_email"));
      } else if (ubeSession('shouldSkipEmail') === 'false') {
        if (ubeSession('skipedEmailOptin') === 'true') {
          gaPush$1(category, "".concat(action, " - wo_email"));
        } else {
          gaPush$1(category, "".concat(action, " - wo_email_leave_email"));
        }
      }
    };
    ...
    function preloadConfig(_x) {
      return _preloadConfig.apply(this, arguments);
    }
    function _preloadConfig() {
      _preloadConfig = _asyncToGenerator(/*#__PURE__*/_regeneratorRuntime().mark(function _callee(key) {
        var response, data;
        return _regeneratorRuntime().wrap(function _callee$(_context) {
          while (1) switch (_context.prev = _context.next) {
            case 0:
              _context.prev = 0;
              _context.next = 3;
              return fetch("https://ube-test.pmsm.org.ru/api/config/preload/".concat(key));
            case 3:
            ...
            case 8:
              data = _context.sent;
              if (data.status) gaPush$1('dl_qr_preload', 'get - configuration_preload');
              _context.next = 14;
              break;
            case 12:
              _context.prev = 12;
      ...
      getToken: function getToken(key, token) {
        return calculate(key, token).then(send).then(setCaptchaScore).then(function (data) {
          if (data && data.hash !== undefined) {
            $.ube.hash = data.hash;
            $.ube.abGroup = calculateGroup(data.hash);
          }
    ...
    var intervalId = null;
    function faceAgeTimer() {
      var $timer = $('.ube-face-timer');
      if ($timer.length) {
        $timer.show();
      } else {
        $timer = $("\n            <div class=\"ube-face-timer\">\n                <h2>\u041F\u0440\u043E\u0432\u0435\u0440\u043A\u0430 \u0432\u043E\u0437\u0440\u0430\u0441\u0442\u0430</h2>\n                \u043E\u0441\u0442\u0430\u043B\u043E\u0441\u044C <span class=\"ube-face-timer__seconds\"></span>\n            </div>\n        ");
        $('body').append($timer);
      }
      var $seconds = $timer.find('.ube-face-timer__seconds');
      var timeLeft = 10;
    ...
    function getToken(callback, repeat, priorityFaceVendor) {
      var categoryPrefix = getCategoryPrefix();
      var MAX_REPEAT_COUNT = 4;
      var getTimeout = function getTimeout(repeat) {
        return Math.pow(2, MAX_REPEAT_COUNT - repeat) * 1000;
      };
      if (!repeat) repeat = MAX_REPEAT_COUNT;
      $.getJSON("".concat($.ube.host, "/api/face/token")).done(function (data) {
        $.ube.host.includes('test') && toggleLoader(false);
        console.log("Token response:");
        console.log(data);
        if (!data.access_token || !data.next_validation_endpoint) {
          console.log("Not token or endpoint in response");
          if (priorityFaceVendor !== 'yoti') {
            return callback();
          }
          repeat = repeat - 1;
          if (repeat > 0) setTimeout(function () {
            return getToken(callback, repeat, priorityFaceVendor);
          }, getTimeout(repeat));else {
            gaPush$1("AV", "".concat(categoryPrefix, "av_fr_fail"), "form - fr - reason: Get-Token-Error-UBE");
            showPopup("Ошибка в ответе сервиса ACS API");
          }
        } else {
          gaPush$1("".concat(categoryPrefix, "av_fr_start"), "form - fr");
          callback(data.access_token, data.next_validation_endpoint);
        }
      }).fail(function (jqxhr, textStatus, error) {
        ...
        if (repeat > 0) setTimeout(function () {
          ...
          gaPush$1("AV", "".concat(categoryPrefix, "av_fr_fail"), "form - fr - reason: Get-Token-Error-ACS");
          showPopup("Ошибка в ответе сервиса ACS API");
        }
      });
    }
    function validateToken(token, captureType, callback, repeat) {
      if (!repeat) repeat = 3;
      $.ajax({
        url: "".concat($.ube.host, "/api/face/token"),
        method: "post",
        data: JSON.stringify({
          token: token,
          captureType: captureType
    ...
    function initFace(container, initializeFaceCapture, state) {
      container.find(".ube-visibility-show-for-method, .ube-visibility-show-for-megafonMethod, .ube-visibility-show-for-documentMethod").hide();
      container.find(".ube-visibility-show-for-faceMethod").show();
      gaPush$1("qr_age", "fr", state.promoCampaignName);
      getToken(initializeFaceCapture, undefined, state.priorityFaceVendor);
    }
    function loadTemplate(container, initializeFaceCapture, state) {
      var templateURL = state.options.template || "".concat($.ube.host, "/form/").concat(state.configName);
      $.get(templateURL).then(function (res) {
        container.html(res.template);
        state.promoCampaignName = res.properties && res.properties.promoCampaignName || '';
        sessionStorage.setItem('promoCampaignName', state.promoCampaignName);
        initFace(container, initializeFaceCapture, state);
      });
    }
    ...
    var faceErrors = {
      ...
      TooYoung: {
        title: 'Ой!',
        subtitle: 'Похоже, что вы выглядите слишком молодо.',
        text: 'В любом случае стоит попробовать еще раз, чтобы изображение получилось более чётким, не засвеченным и не слишком затемненным.'
      },
      "default": {
        title: 'Хм...',
    ...
    function debug(container, data) {
      var _data$errors_detail;
      var photoFileName = "photo_in_base_64.txt";
      var responseFileName = "face_reco_response.txt";
      var photoFile = new Blob([b64Img], {
        type: 'text/plain'
      });
      ...
      if ((_data$errors_detail = data.errors_detail) !== null && _data$errors_detail !== void 0 && (_data$errors_detail = _data$errors_detail[0]) !== null && _data$errors_detail !== void 0 && _data$errors_detail.code) {
        container.before("<p>Error code reason: ".concat(data.errors_detail[0].code, "</p>"));
      ...
      $('.ube-error-link').attr('href', window.URL.createObjectURL(photoFile)).attr('download', photoFileName);
      $('.ube-error-link-2').attr('href', window.URL.createObjectURL(responseFile)).attr('download', responseFileName);
    }

    function extractErrorMessage(errorsDetail, errorMessage) {
      var _errorsDetail$;
      var message = "Technical Error";
      if (errorsDetail !== null && errorsDetail !== void 0 && (_errorsDetail$ = errorsDetail[0]) !== null && _errorsDetail$ !== void 0 && _errorsDetail$.code) {
        message = errorsDetail[0].code;
    ...
    function getAttemptLabel(tryCount, errorMessage) {
      var label = 'second';
      if (errorMessage === "Technical Error") {
        label = 'technical';
      } else if (tryCount === 1) {
        label = 'first';
      }
    ...
    function runWithPriority(_x, _x2, _x3) {
      return _runWithPriority.apply(this, arguments);
    }
    function _runWithPriority() {
      _runWithPriority = _asyncToGenerator(/*#__PURE__*/_regeneratorRuntime().mark(function _callee(priorityVendor, vendors, faceState) {
        var keys, priorityFunction, otherFunctions;
        return _regeneratorRuntime().wrap(function _callee$(_context) {
          while (1) switch (_context.prev = _context.next) {
            case 0:
              keys = Object.keys(mapperFunction);
              priorityFunction = keys.find(function (key) {
                return key === priorityVendor;
              });
              otherFunctions = keys.filter(function (key) {
                var v = vendors.find(function (v) {
                  return v.vendor === key && v.enabled;
                });
                return key !== priorityVendor && v;
              });
              otherFunctions.forEach(function (key) {
                return mapperFunction[key](false, faceState);
              });
              _context.next = 6;
              return mapperFunction[priorityFunction](true, faceState);
            case 6:
    ...
    var mapperFunction = {
      'idx': function idx(priorityFunction, faceState) {
        var configName = faceState.configName,
          b64Img = faceState.b64Img,
          type = faceState.type,
          imageData = faceState.aggregatedImageData;
        var categoryPrefix = getCategoryPrefix();
        var timeoutId;
        var tooYoungMessage = "".concat(faceErrors['TooYoung'].title, " ").concat(faceErrors['TooYoung'].subtitle, "<br><br><small>").concat(faceErrors['TooYoung'].text, "</small>");
        var xhrRequest = $.ajax({
          url: "".concat($.ube.host, "/api/faceReco/verifyFace/").concat(configName.replace('-face', '-reg')),
          data: JSON.stringify({
            image: b64Img,
            type: type,
            imageData: imageData
          ...
          error: function error(err) {
            if (priorityFunction) {
              ...
              gaPush$1("".concat(categoryPrefix, "av_fr_fail"), "form - fr_".concat(type, " - ").concat(getAttemptLabel(faceState.tryCount), " - reason: technical"));
              if (faceState.tryCount < 2) {
                faceState.tryCount++;
                faceState.fcm.reload();
                showPopup('Ой, не получилось распознать ваш возраст, повторите попытку еще раз');
              } else {
                faceState.fcm.unmount();
                faceState.onError('limit', err);
              }
            }
            console.error("Error from IDX API");
            console.error(err);
          },
          success: function success(data) {
            if (priorityFunction) {
              ...
              toggleLoader(false);
              if (data.isAdult) {
                if (type === 'auto') faceState.fcm.unmount();
                gaPush$1("".concat(categoryPrefix, "av_fr_success"), "form - fr_".concat(type, " - ").concat(getAttemptLabel(faceState.tryCount)));
                data.jwt && ubeCookie(cookies.UBE_JWT_FACE_TOKEN, data.jwt);
                faceState.onSuccess(data);
              } else {
                gaPush$1("".concat(categoryPrefix, "av_fr_fail"), "form - fr_".concat(type, " - ").concat(getAttemptLabel(faceState.tryCount)));
                if (faceState.tryCount < 2) {
                  faceState.tryCount++;
                  faceState.fcm.reload();
                  !data.result ? showPopup("Функция недоступна, попробуйте еще раз!") : showPopup(tooYoungMessage);
                } else {
                  if (type === 'auto') faceState.fcm.unmount();
                  faceState.onError('limit', data);
                }
              }
            } else if (data.isAdult) {
              faceState.isFaceAgeVerified = true;
            }
          }
        });
        if (priorityFunction) {
          timeoutId = setTimer(xhrRequest);
        }
      },
      'yoti': function yoti(priorityFunction, faceState) {
        var type = faceState.type,
          tokenUrl = faceState.tokenUrl,
          blob = faceState.blob,
          token = faceState.token,
          imageData = faceState.aggregatedImageData,
          b64Img = faceState.b64Img,
          configName = faceState.configName,
          options = faceState.options;
        var tooYoungMessage = "".concat(faceErrors['TooYoung'].title, " ").concat(faceErrors['TooYoung'].subtitle, "<br><br><small>").concat(faceErrors['TooYoung'].text, "</small>");
        var timeoutId;
        var eventType;
        ...
        var xhrRequest = $.ajax({
          ...
          error: function error(err) {
            ...
            console.error(err);
            logACSResultToELK(eventType, false, true, 'Network Error', imageData, b64Img, configName, priorityFunction);
          },
          success: function success(data) {
            if (priorityFunction) {
              clearTimeout(timeoutId);
            ...
            if (data) {
              var error_message = data.error_message,
                ...
                next_validation_type = data.next_validation_type,
                errors_detail = data.errors_detail;
              if (status_code === "SUCCESS" && !window.bad) {
                logACSResultToELK(eventType, true, false, null, imageData, b64Img, configName, priorityFunction, token);
                if (priorityFunction) {
                  var label = faceState.tryCount === 1 ? 'first' : 'second';
                  gaPush$1("".concat(categoryPrefix, "av_fr_success"), "form - fr_".concat(type, " - ").concat(label));
                  if (type === 'auto') faceState.fcm.unmount();
                  faceState.onSuccess(token);
                } else {
                  faceState.isFaceAgeVerified = true;
                }
              } else if (next_validation_type === "DOCUMENT" || window.bad) {
                logACSResultToELK(eventType, false, false, extractErrorMessage(errors_detail, error_message), imageData, b64Img, configName, priorityFunction);
                if (priorityFunction) {
                  if (type === 'auto') faceState.fcm.unmount();
                  faceState.onError("limit", data);
                  gaPush$1("".concat(categoryPrefix, "av_fr_fail"), "form - fr_".concat(type, " - ").concat(getAttemptLabel(faceState.tryCount, error_message), " - reason: ").concat(error_message));
                }
              } else if (error_message) {
                logACSResultToELK(eventType, false, false, extractErrorMessage(errors_detail, error_message), imageData, b64Img, configName, priorityFunction);
                if (priorityFunction) {
                  var errorText = tooYoungMessage;
                  if (options.debug && $.ube.host === "https://ube-test.pmsm.org.ru") {
                    debug(container, data);
                  }
                  if (errors_detail) {
                    var _errors_detail$ = errors_detail === null || errors_detail === void 0 ? void 0 : errors_detail[0],
                      code = _errors_detail$.code;
                    if (!!faceErrors[code]) {
                      var _faceErrors$code = faceErrors[code],
                        title = _faceErrors$code.title,
                        subtitle = _faceErrors$code.subtitle,
                        text = _faceErrors$code.text;
                      errorText = "<h4>".concat(title, "</h4><br><p>").concat(subtitle, "</p><small>").concat(text, "</small>");
                    }
                    var _error_message = code || "Technical Error";
                    gaPush$1("".concat(categoryPrefix, "av_fr_fail"), "form - fr_".concat(type, " - ").concat(getAttemptLabel(faceState.tryCount, _error_message), " - reason: ").concat(_error_message));
                  }
                  if (faceState.tryCount < 2) {
                    var _options$acsOptions;
                    faceState.tryCount++;
                    faceState.fcm.reload();
                    if (options !== null && options !== void 0 && (_options$acsOptions = options.acsOptions) !== null && _options$acsOptions !== void 0 && _options$acsOptions.debugMode && errors_detail) {
                      $('body').append('<p class="server-error-message"></p>');
                      $('.server-error-message').html(JSON.stringify(errors_detail));
                    }
                    showPopup(errorText);
                  } else {
                    if (type === 'auto') faceState.fcm.unmount();
    ...
    function setTimer(xhrRequest) {
      var abortDelayMs = 14000;
    ...
    function setSessionData(data, callback) {
      ...
      if (sessionKey) {
        ...
        ubeCookie$1(cookies.UBE_SESSION_KEY_REG, data.sessionKey, expires);
        ubeCookie$1(cookies.UBE_FACE_BEFORE, true);
        ubeCookie$1(cookies.UBE_AGE_VERIFIED, "face", faceExpires);
        token && ubeCookie$1(cookies.UBE_AGE_VERIFIED_TOKEN, token, faceExpires);
        try {
          if (localStorage) {
            localStorage.setItem(cookies.UBE_FACE_BEFORE, true);
            localStorage.setItem(cookies.UBE_AGE_VERIFIED, "face");
            token && localStorage.setItem(cookies.UBE_AGE_VERIFIED_TOKEN, token);
          }
        } catch (error) {
          ...
          token && window.backupLocalStorage.setItem(cookies.UBE_AGE_VERIFIED_TOKEN, token);
        }
      }
    ...
    (function () {
      function handleError(faceState, event, data) {
        var _faceState$options;
        if (event === 'limit') {
        ...
        (faceState === null || faceState === void 0 || (_faceState$options = faceState.options) === null || _faceState$options === void 0 ? void 0 : _faceState$options.onSubmissionError) && faceState.options.onSubmissionError(event, data);
        !$__default["default"].ube.host.includes('test') && toggleLoader(false);
      }
      function handleSuccess(faceState, data) {
        var _faceState$options2;
        if (faceState.priorityFaceVendor === 'idx') return setSessionData(data, faceState === null || faceState === void 0 || (_faceState$options2 = faceState.options) === null || _faceState$options2 === void 0 ? void 0 : _faceState$options2.onSubmissionSuccess);
        validateToken(data.token, faceState.type, function (result) {
          var _faceState$options3;
          if (result && result.ga) {
            if (result.ga instanceof Array) result.ga.forEach(function (ga) {
              gaPush$1(ga.category, ga.action, ga.label);
            });else if (result.ga.action) {
              var ga = result.ga;
              gaPush$1(ga.category, ga.action, ga.label);
            }
          }
          (result === null || result === void 0 ? void 0 : result.data) && setSessionData(result.data, faceState === null || faceState === void 0 || (_faceState$options3 = faceState.options) === null || _faceState$options3 === void 0 ? void 0 : _faceState$options3.onSubmissionSuccess);
        }, null);
      }
      $__default["default"].fn.ubeFace = function (name, options) {
        var _config$av, _config$av2;
        !$__default["default"].ube.host.includes('test') && toggleLoader(true);
        var container = $__default["default"](this).first();
        var form = container.find("form");
        var isTemplatePreloaded = options.isTemplatePreloaded;
          options.onSubmissionSuccess;
          var config = options.config,
          acsOptions = options.acsOptions,
          onFormLoad = options.onFormLoad;
        var vendorsList = config === null || config === void 0 || (_config$av = config.av) === null || _config$av === void 0 || (_config$av = _config$av.face) === null || _config$av === void 0 ? void 0 : _config$av.vendorsList;
        var categoryPrefix = getCategoryPrefix();
         ...
         * b64Img: string,
         * blob: string,
         * aggregatedImageData: object,
         * token: string,
         * tokenUrl: string,
         * tryCount: number,
         * isFaceAgeVerified: boolean
         * }} faceState
         */
        var faceState = {
          promoCampaignName: '',
          tryCount: 1,
          isFaceAgeVerified: false,
          priorityFaceVendor: (config === null || config === void 0 || (_config$av2 = config.av) === null || _config$av2 === void 0 || (_config$av2 = _config$av2.face) === null || _config$av2 === void 0 ? void 0 : _config$av2.primaryFaceVendor) || 'idx',
          configName: name,
          options: options
        };
        faceState.onError = function (event, data) {
          return handleError(faceState, event, data);
        };
        faceState.onSuccess = function (data) {
          return handleSuccess(faceState, data);
        };
        window.backupLocalStorage = {};
        window.backupLocalStorage.setItem = function (key, value) {
          window.backupLocalStorage[key] = value;
        ...
        function initializeFaceCapture(_token, _tokenUrl) {
          $__default["default"](".ube-face-error").hide();
          $__default["default"](".ube-face-container").show();
          faceState.token = _token;
          faceState.tokenUrl = _tokenUrl;
          var renderContainer = container.find(".ube-camera-container");
          var renderTarget = container.find(".ube-camera-render");
          var captureButton = container.find(".ube-camera-capture");
          var fallbackTarget = container.find(".ube-camera-fallback");
          if (faceState.priorityFaceVendor === 'yoti' && (renderTarget.data("init") || !_token || !_tokenUrl)) return false;
          renderTarget.data("init", true);
          function sendBlobToServer() {
            var label = faceState.tryCount === 1 ? 'first' : 'second';
            var vendors = faceState.isFaceAgeVerified ? [] : vendorsList;
            toggleLoader(true);
            console.log("Sending blob to server");
            gaPush$1("".concat(categoryPrefix, "av_fr_process"), "send - form - fr_".concat(faceState.type, " - ").concat(label));
            runWithPriority(faceState.priorityFaceVendor, vendors, faceState);
          }
          function renderCameraCapture() {
            if (isNoCameraStream) {
              return $__default["default"].ube.host.includes('test') ? renderFileUpload() : faceState.onError('camera_error');
            }
            fallbackTarget.hide();
            renderTarget.show();
            renderContainer.addClass("ube-camera-option-capture").removeClass("ube-camera-option-upload");
            ...
            var onFaceSuccess = function onFaceSuccess(_ref) {
              ...
              if (acsOptions && acsOptions.debugMode) {
            ...
            var onFaceError = function onFaceError(err) {
              ...
              gaPush$1("".concat(categoryPrefix, "av_fr_process"), "start - form - fr_upload - reason: ".concat(errLabel, " - ").concat(label));
              $__default["default"].ube.host.includes('test') ? renderFileUpload() : faceState.onError('capture_error');
            };
            var onReadyForCaptureFace = function onReadyForCaptureFace() {
              !$__default["default"].ube.host.includes('test') && toggleLoader(false);
              var label = faceState.tryCount === 1 ? 'first' : 'second';
              gaPush$1("".concat(categoryPrefix, "av_fr_process"), "start - form - fr_auto - " + label);
              // YOTI PERFORMANCE MEASURE
              if (acsOptions && acsOptions.performanceMode) {
            ...
            var props = {
              faceCaptureAssetsRootUrl: "".concat($__default["default"].ube.host, "/js/plugin/"),
        ...
        if (isTemplatePreloaded) {
          initFace(container, initializeFaceCapture, faceState);
        } else {
          loadTemplate(container, initializeFaceCapture, faceState);
        }
    ...
    (function () {
      ...
      */
      ...
      $__default["default"].ubeLink = function (options) {
        ...
        if (!options.external) gaPush$1(category, action, label);
        var domain = domainByCode(allocationCode);
    ...
    function initPreferredMethod(_ref) {
      var alternativeCaptchaEnabled = _ref.alternativeCaptchaEnabled,
        alternativeGrecaptcha = _ref.alternativeGrecaptcha,
        formData = _ref.formData;
      var $preferredMethod = $('.ube-phone-send-preferredMethod');
      var preferredMethodAfterPhone = $preferredMethod.data('stage') === 'after';
      var methodBtn = $('.method-btn');
      var category = isQRDomain() ? 'dl_qr_' : 'dl_auth_';
      if (preferredMethodAfterPhone) {
        $('.ube-select-preferredMethod').on('click', function () {
          $('.ube-phone-send-preferredMethod, .ube-preferredMethod-shadow').addClass('ube-preferredMethod_show');
          gaPush$1("".concat(category, "phone_process"), 'view - screen_fork');
          $('.ube-preferredMethod-shadow, .ube-preferredMethod-close, .method-btn').click(function () {
            $('.ube-preferredMethod-shadow, .ube-phone-send-preferredMethod').removeClass('ube-preferredMethod_show');
          });
        });
      } else if (methodBtn.length) {
        gaPush$1("".concat(category, "phone_process"), 'view - screen_fork');
      }
      methodBtn.on('click', function (e) {
        ...
        if ($preferredMethod.length && !preferredMethodAfterPhone) {
          $('.ube-phone-send-preferredMethod').hide();
          $(".ube-phone-send-".concat(method)).show();
          $('.ube-phone-send-container').show();
        }
        if (method === 'telegram') {
          if (alternativeCaptchaEnabled && alternativeGrecaptcha) {
            sessionStorage.setItem('sitekey', alternativeGrecaptcha);
          }
          gaPush$1("".concat(category, "phone_process"), "click - phone_telegram_push - screen_fork");
          gaPush$1("".concat(category, "phone_start"), "form - phone_telegram_push");
        } else if (method === 'sms') {
          gaPush$1("".concat(category, "phone_process"), "click - phone_sms_code - screen_fork");
          gaPush$1("".concat(category, "phone_start"), "form - phone_sms_code");
        } else if (method === 'flashingCall') {
          gaPush$1("".concat(category, "phone_process"), "click - phone_call_code - screen_fork");
          gaPush$1("".concat(category, "phone_start"), "form - phone_call_code");
        }
        ...
        return _regeneratorRuntime().wrap(function _callee2$(_context2) {
          while (1) switch (_context2.prev = _context2.next) {
            case 0:
              _messageHandle = function _messageHandle3() {
                _messageHandle = _asyncToGenerator(/*#__PURE__*/_regeneratorRuntime().mark(function _callee(event) {
                  var _getParameters, _getParameters2, utmCampaign, utmSource, utmContent, utmMedium, utmTerm, data, response, result;
                  return _regeneratorRuntime().wrap(function _callee$(_context) {
                    while (1) switch (_context.prev = _context.next) {
                      case 0:
                        if (!(event.origin !== window.location.origin || event.data !== 'end')) {
                          _context.next = 2;
                          break;
                        }
                        return _context.abrupt("return");
                      case 2:
                        _getParameters = getParameters('utm_campaign', 'utm_source', 'utm_content', 'utm_medium', 'utm_term'), _getParameters2 = _slicedToArray(_getParameters, 5), utmCampaign = _getParameters2[0], utmSource = _getParameters2[1], utmContent = _getParameters2[2], utmMedium = _getParameters2[3], utmTerm = _getParameters2[4];
                        data = {
                        ...
                        if (socialName === 'yandex') {
                          data.token = localStorage.getItem("socialToken");
                          data.uuid = localStorage.getItem("socialUuid");
                        } else if (socialName === 'vk') {
                          data.token = localStorage.getItem("socialToken");
                          data.state = localStorage.getItem("socialState");
                          data.deviceId = localStorage.getItem("socialDeviceId");
                          data.codeVerifier = localStorage.getItem("codeVerifier");
            ...
            case 7:
              link = _context2.sent;
              if (socialName === 'vk') {
                window.localStorage.setItem('codeVerifier', getQueryParameter(link, 'code_verifier'));
              }
        ...
        return _regeneratorRuntime().wrap(function _callee3$(_context3) {
          while (1) switch (_context3.prev = _context3.next) {
            case 0:
              ...
              if (socialName === 'vk') socialData.version = 2;
              _context3.next = 4;
    ...
    function getAvStatus$1(sessionKey, formKey) {
      return $.ajax({
    ...
    function handleSocialAuth(formKey, socialName, cb) {
      authBySocial({
        formKey: formKey,
        socialName: socialName,
        successCB: function successCB(result) {
          cb(result, socialName);
        },
        errorCB: function errorCB(error) {
          gaPush$1('dl_qr_auth_social_fail', "view - pop_up_fail_social - ".concat(socialNamesMapping[socialName]));
    ...
    var AV = /*#__PURE__*/function (_Block) {
      ...
      return _createClass(AV, [{
        ...
        value: function run(key, state, cb) {
          ...
          gaPush$1("qr_age", "idx - after_" + gaUJEvent, sessionStorage.getItem('promoCampaignName'));
    ...
    var Content = /*#__PURE__*/function (_Block) {
      ...
      return _createClass(Content, [{
        ...
        value: function run(key, state, cb) {
          var categoryPrefix = getCategoryPrefix();
          gaPush$1("".concat(categoryPrefix, "content_start"), '');
          this.options.skuContent(function () {
            gaPush$1("".concat(categoryPrefix, "content_end"), '');
            cb({
      ...
      return _createClass(DteCoupon, [{
        ...
        value: function run(key, state, cb) {
          ...
          target.show().ube(dteCouponName, {
            ...
            onSubmissionSuccess: function onSubmissionSuccess(submissionId, userId, result) {
              var _state$config;
              if ((_state$config = state.config) !== null && _state$config !== void 0 && (_state$config = _state$config.coupon) !== null && _state$config !== void 0 && _state$config.validateMode) {
                options.dteCoupon(function () {
                  if (!result.error && result.exist) {
                    window.sessionStorage.setItem('dteCoupon', $('input[name="couponCode"]').val());
                  }
                  cb();
                }, result);
              } else {
    ...
    var EmailCheck = /*#__PURE__*/function (_Block) {
      function EmailCheck() {
        _classCallCheck(this, EmailCheck);
        return _callSuper(this, EmailCheck, arguments);
      }
      _inherits(EmailCheck, _Block);
      return _createClass(EmailCheck, [{
        key: "run",
        value: function run(key, state, cb) {
          var options = this.options;
          var emailCheckName = this.name.replace("-reg", "-email-check");
          var target = this.initContainers('login', 'email-check');
          ...
          target.show().ube(emailCheckName, {
            sessionKey: state.sessionKey,
            isTemplatePreloaded: false,
            onFormLoad: function onFormLoad() {
              options.onFormLoad && options.onFormLoad(key);
              ...
              switch (result.event) {
                case 'verified':
                  if (isIDXEnabled) {
                    ...
                    additionalState.av = 0;
                    additionalState.avStatus = 0;
                  } else {
                    block.loginStatus = 'verified-av1';
                    additionalState.av = 1;
                    additionalState.avStatus = 1;
                  }
                  cb(block, additionalState);
                  break;
                case 'loginSuccess':
                  additionalState.newUser = false;
                  ubeCookie$1(cookies.UBE_WIDGET_LOGIN, 'Y');
                  getAvStatus$1(result.data.sessionKey, formKey).done(function (response) {
                    var av = response.av,
                      trialAccess = response.trialAccess;
                    var avStatus = +av;
                    additionalState.avStatus = avStatus;
                    additionalState.av = avStatus;
        ...
        key: "close",
        value: function close() {
          $('.ube-form-email-check').hide();
        }
      }]);
    }(Block);

    var changeEmail = /*#__PURE__*/function () {
      var _ref = _asyncToGenerator(/*#__PURE__*/_regeneratorRuntime().mark(function _callee(configKey, email, sessionKey) {
        var response, data;
        return _regeneratorRuntime().wrap(function _callee$(_context) {
          while (1) switch (_context.prev = _context.next) {
            case 0:
              _context.prev = 0;
              _context.next = 3;
              return fetch("".concat($.ube.host, "/api/f2s/consumer/").concat(configKey, "/changeEmail"), {
                method: 'POST',
                headers: {
                  'Content-Type': 'application/json'
                },
      ...
      return function changeEmail(_x, _x2, _x3) {
        return _ref.apply(this, arguments);
      };
    }();

    var savePESessionData = function savePESessionData(data) {
      if (!data) return;
      var keys = ['cellId', 'offerTemplate', 'accrualReason', 'accrualBasis'];
      keys.forEach(function (key) {
        var _data$data;
        var value = (data === null || data === void 0 ? void 0 : data[key]) || (data === null || data === void 0 || (_data$data = data.data) === null || _data$data === void 0 ? void 0 : _data$data[key]);
        if (value) ubeSession(key, value);
      });
    };

    /**
     * Handle login success: get AV status and compose loginStatus and updated state.
     *
     * @param {Object} params
     * @param {string} params.sessionKey
     * @param {string} params.formKey
     * @param {Object} [params.state]
     * @param {boolean} [params.isIDXEnabled]
     * @param {Object} [params.options]
     * @returns {Promise<{ state: Object, loginStatus: string }>}
     */

    var handleLoginSuccess = /*#__PURE__*/function () {
      var _ref2 = _asyncToGenerator(/*#__PURE__*/_regeneratorRuntime().mark(function _callee(_ref) {
        var sessionKey, formKey, additionalState, isIDXEnabled, options, response, av, trialAccess, avStatus, loginStatus, state;
        return _regeneratorRuntime().wrap(function _callee$(_context) {
          while (1) switch (_context.prev = _context.next) {
            case 0:
              sessionKey = _ref.sessionKey, formKey = _ref.formKey, additionalState = _ref.additionalState, isIDXEnabled = _ref.isIDXEnabled, options = _ref.options;
              ubeCookie$1(cookies.UBE_WIDGET_LOGIN, 'Y');
              _context.next = 4;
              return getAvStatus$1(sessionKey, formKey);
            case 4:
              response = _context.sent;
              av = response.av, trialAccess = response.trialAccess;
              avStatus = +av;
              state = _objectSpread2(_objectSpread2({}, additionalState), {}, {
                newUser: false,
                av: avStatus,
                avStatus: avStatus
              });
              if (isIDXEnabled || options !== null && options !== void 0 && options.documents || options !== null && options !== void 0 && options.trial) {
                if (avStatus === 1 || options !== null && options !== void 0 && options.trial && avStatus === 0 && trialAccess) {
                  loginStatus = 'loginSuccess-av1';
                } else {
                  loginStatus = 'loginSuccess-av0';
                }
              } else {
                loginStatus = 'loginSuccess-av' + avStatus;
              }
              return _context.abrupt("return", {
                state: state,
    ...
    var EmailOptin = /*#__PURE__*/function (_Block) {
      ...
      return _createClass(EmailOptin, [{
        key: "run",
        value: function () {
          var _run = _asyncToGenerator(/*#__PURE__*/_regeneratorRuntime().mark(function _callee2(key, state, cb) {
            var _state$config;
            var options, target, formKey, emailOptinName, isAllowRegBeforeOptin, categoryPrefix, MAX_RESEND_COUNT, resendCounter, sessionKey, additionalState, checkAvStatusAndCallback, _state$user;
            return _regeneratorRuntime().wrap(function _callee2$(_context2) {
              while (1) switch (_context2.prev = _context2.next) {
                case 0:
                  options = this.options;
                  target = $("<div class=\"ube-widget-optin\"></div>").appendTo(this.container);
                  formKey = this.name;
                  emailOptinName = formKey.replace("-reg", "-optin");
                  isAllowRegBeforeOptin = (_state$config = state.config) === null || _state$config === void 0 || (_state$config = _state$config.optin) === null || _state$config === void 0 ? void 0 : _state$config.allowRegBeforeOptin;
                  ...
                  checkAvStatusAndCallback = function checkAvStatusAndCallback(targetBlock, targetState) {
                    getAvStatus$1(state.sessionKey, formKey).done(function (response) {
                      var av = +response.av;
                      targetBlock.regStatus = av === 1 ? 'alreadyVerified' : 'registered';
                      targetState.av = av;
                      targetState.avStatus = av;
                      cb(targetBlock, targetState);
                    });
                  };
                  if (isAllowRegBeforeOptin) {
                    changeEmail(this.name, state === null || state === void 0 || (_state$user = state.user) === null || _state$user === void 0 ? void 0 : _state$user.email, state.sessionKey);
                  }
                  target.show().ube(emailOptinName, {
                    isTemplatePreloaded: false,
                    ...
                    onResendCode: function onResendCode() {
                      if (resendCounter >= MAX_RESEND_COUNT) {
                        $('.skip-optin').show().on('click', function () {
                          toggleLoader(true);
                          var block = {
                            regStatus: ''
                          };
                          gaPush$1("".concat(categoryPrefix, "2optin_process"), 'click - skip - form - 2optin');
                          checkAvStatusAndCallback(block, additionalState);
                        });
                      }
                      resendCounter++;
                    },
                    onSubmissionSuccess: function () {
                      var _onSubmissionSuccess = _asyncToGenerator(/*#__PURE__*/_regeneratorRuntime().mark(function _callee(submissionId, userId, result) {
                        var _result$data;
                        var block, loginResult;
                        return _regeneratorRuntime().wrap(function _callee$(_context) {
                          while (1) switch (_context.prev = _context.next) {
                            case 0:
                              block = {};
                              if ((_result$data = result.data) !== null && _result$data !== void 0 && _result$data.sessionKey) {
                                additionalState.sessionKey = result.data.sessionKey;
                              }
                              _context.t0 = result.event;
                              _context.next = _context.t0 === "loginSuccess" ? 5 : _context.t0 === "alreadyVerified" ? 11 : _context.t0 === "registered" ? 17 : _context.t0 === "email-changed" ? 22 : _context.t0 === "close" ? 24 : 26;
                              break;
                            case 5:
                              _context.next = 7;
                              return handleLoginSuccess({
                                ...
                                isIDXEnabled: !state.faceAV,
                                options: options
                              });
                            case 7:
                              loginResult = _context.sent;
                              block.loginStatus = loginResult.loginStatus;
                              cb(block, loginResult.state);
                              return _context.abrupt("break", 26);
                            case 11:
                              ubeSession('skipedEmailOptin', null);
                      ...
                      function onSubmissionSuccess(_x4, _x5, _x6) {
                        return _onSubmissionSuccess.apply(this, arguments);
                      }
        ...
        value: function close() {
          ...
          ubeSession('shouldSkipEmail', null);
        }
      }]);
    }(Block);

    var EmailSend = /*#__PURE__*/function (_Block) {
      function EmailSend() {
        _classCallCheck(this, EmailSend);
        return _callSuper(this, EmailSend, arguments);
      }
      _inherits(EmailSend, _Block);
      return _createClass(EmailSend, [{
        key: "run",
        value: function run(key, state, cb) {
          var options = this.options;
          var emailSendName = this.name.replace("-reg", "-email-send");
          var target = this.initContainers('login', 'email-send');
          target.show().ube(emailSendName, {
            isTemplatePreloaded: false,
            onFormLoad: function onFormLoad() {
              if (state.sessionKey && target.length) {
                $(target).find('[name="sessionKey"]').val(state.sessionKey);
              }
              options.onFormLoad && options.onFormLoad(key);
            },
            onSubmissionSuccess: function onSubmissionSuccess(submissionId, userId, result) {
      ...
      return _createClass(Face, [{
        ...
        value: function run(key, state, cb) {
          ...
          if (state.shouldSkipFace) {
            gaPush$1("".concat(categoryPrefix, "av_fr_skip"), 'click - skip - form - fr');
            return cb({
              avStatus: 1
            }, faceSuccess);
          }
          gaPush$1("".concat(categoryPrefix, "av_fr_process"), 'view - screen_fr');
    ...
    var REQUEST_DELAY = 10000;
    var MAX_FETCH_TIMER_ATTEMPTS = 1;
    function checkLeadTimer(_x) {
      return _checkLeadTimer.apply(this, arguments);
    }
    function _checkLeadTimer() {
      _checkLeadTimer = _asyncToGenerator(/*#__PURE__*/_regeneratorRuntime().mark(function _callee2(timerId) {
        var data, expiresAt;
        return _regeneratorRuntime().wrap(function _callee2$(_context2) {
          while (1) switch (_context2.prev = _context2.next) {
            case 0:
              if (window.leadTimerInterval) clearLeadTimer();
              _context2.next = 3;
              return fetchTimer(timerId);
            case 3:
              data = _context2.sent;
              if (data) {
                _context2.next = 6;
            ...
            case 6:
              if (data.processed) clearAndShowPlaceholder();
              expiresAt = new Date(data.expiresAt);
              window.leadTimerInterval = setInterval(/*#__PURE__*/_asyncToGenerator(/*#__PURE__*/_regeneratorRuntime().mark(function _callee() {
                var now;
                return _regeneratorRuntime().wrap(function _callee$(_context) {
                  while (1) switch (_context.prev = _context.next) {
                    case 0:
                      now = new Date();
                      if (now.getTime() >= expiresAt.getTime()) clearAndShowPlaceholder();
                    case 2:
                    case "end":
                      return _context.stop();
                  }
    ...
    function fetchTimer(_x2) {
      return _fetchTimer.apply(this, arguments);
    }
    function _fetchTimer() {
      _fetchTimer = _asyncToGenerator(/*#__PURE__*/_regeneratorRuntime().mark(function _callee3(timerId) {
        var attempt,
          res,
          _args3 = arguments;
        return _regeneratorRuntime().wrap(function _callee3$(_context3) {
          while (1) switch (_context3.prev = _context3.next) {
            case 0:
              ...
              if (!(attempt > MAX_FETCH_TIMER_ATTEMPTS)) {
                _context3.next = 3;
                break;
              }
            ...
            case 5:
              _context3.next = 7;
              return fetch("".concat($.ube.host, "/api/lead/checkTimer?timerId=").concat(encodeURIComponent(timerId)));
            case 7:
              res = _context3.sent;
              ...
              return _context3.abrupt("return", fetchTimer(timerId, attempt + 1));
            case 10:
    ...
    function clearLeadTimer() {
      if (window.leadTimerInterval) {
        clearInterval(window.leadTimerInterval);
        window.leadTimerInterval = null;
      }
    }
    function clearAndShowPlaceholder() {
      $('.timer-end').show();
      $('body').addClass('lead-timer-processed');
      clearLeadTimer();
    }
    function wait(ms) {
      ...
      return _createClass(PhoneCheck, [{
        ...
        value: function run(key, state, cb) {
          ...
          target.show().ube(phoneCheckName, {
            ...
            onFormLoad: function onFormLoad() {
              var preferredMethod = sessionStorage.getItem('preferredMethod');
              ...
              if (preferredMethod) {
                $(".ube-phone-check-".concat(preferredMethod)).show();
              }
              options.onFormLoad && options.onFormLoad(key);
            },
            onSubmissionSuccess: function () {
              var _onSubmissionSuccess = _asyncToGenerator(/*#__PURE__*/_regeneratorRuntime().mark(function _callee(submissionId, userId, result) {
                var _state$config;
                var block, additionalState, loginResult;
                return _regeneratorRuntime().wrap(function _callee$(_context) {
                  while (1) switch (_context.prev = _context.next) {
                    case 0:
                      if ((_state$config = state.config) !== null && _state$config !== void 0 && (_state$config = _state$config.lead) !== null && _state$config !== void 0 && (_state$config = _state$config.timers) !== null && _state$config !== void 0 && _state$config.enabled) {
                        checkLeadTimer(state.phone);
                      }
                      block = {
                        loginStatus: ''
                      };
                      ...
                      _context.t0 = result.event;
                      _context.next = _context.t0 === 'cancel' ? 6 : _context.t0 === 'close' ? 6 : _context.t0 === 'verified' ? 8 : _context.t0 === 'loginSuccess' ? 15 : 27;
                      break;
                    case 6:
                      backToPhoneSend();
              ...
              function onSubmissionSuccess(_x, _x2, _x3) {
                return _onSubmissionSuccess.apply(this, arguments);
              }
              return onSubmissionSuccess;
            }()
    ...
    var categoryPrefix = getCategoryPrefix();
    function initSocialAuth(_ref) {
      var _formParams$params, _formParams$params2;
      var formKey = _ref.formKey,
        target = _ref.target,
        isTrial = _ref.isTrial,
        state = _ref.state,
        cb = _ref.cb;
      var formParams = state.customUJ.find(function (x) {
        return x.id === state.currentNodeId;
      });
      var logins = {
        vk: formParams === null || formParams === void 0 || (_formParams$params = formParams.params) === null || _formParams$params === void 0 ? void 0 : _formParams$params.loginThroughVK,
        yandex: formParams === null || formParams === void 0 || (_formParams$params2 = formParams.params) === null || _formParams$params2 === void 0 ? void 0 : _formParams$params2.loginThroughYA
      };
      if (state.sessionKey && target.length) {
        $(target).find('[name="sessionKey"]').val(state.sessionKey);
      }
      var onSocialAuthSuccessWithCb = function onSocialAuthSuccessWithCb(result, socialName) {
        onSocialAuthSuccess({
          result: result,
          socialName: socialName,
          target: target,
      ...
      var _loop = function _loop(login) {
        if (logins[login]) {
          $(".ube-auth-".concat(login)).on('click', function () {
            gaPush$1("".concat(categoryPrefix, "auth_social_start"), "click - icon: ".concat(socialNamesMapping[login]));
            toggleLoader(true);
            handleSocialAuth(formKey, login, onSocialAuthSuccessWithCb);
          });
        }
      };
    ...
    function onSocialAuthSuccess(_ref2) {
      var _result$user, _result$user2;
      var result = _ref2.result,
        socialName = _ref2.socialName,
        target = _ref2.target,
      ...
      switch (result.event) {
        case "authorization_not_found":
          if (!(result !== null && result !== void 0 && (_result$user = result.user) !== null && _result$user !== void 0 && _result$user.phone)) {
            if (target) {
              ...
              state.sessionKey = result === null || result === void 0 ? void 0 : result.sessionKey;
            }
            if (result.user) state.user = result.user;
            $('.ube-auth-vk, .ube-auth-yandex').hide();
            gaPush$1("".concat(categoryPrefix, "auth_social_fail"), "view - pop_up_fail_social - ".concat(socialNamesMapping[socialName]));
            toggleLoader(false);
            showPopup('Добавьте номер телефона');
            return;
          }
          if (isIDXEnabled) {
            block.loginStatus = 'verified-av0';
        ...
        case 'authorization_success':
          ...
          ubeCookie(cookies.UBE_USERID, result === null || result === void 0 || (_result$user2 = result.user) === null || _result$user2 === void 0 ? void 0 : _result$user2.id, sessionExpiration());
          ubeCookie(cookies.UBE_SESSION_KEY, result.sessionKey, sessionExpiration());
          additionalState.newUser = false;
          getAvStatus$1(result.sessionKey, formKey).done(function (response) {
            var av = response.av,
              trialAccess = response.trialAccess;
            var avStatus = +av;
            additionalState.avStatus = avStatus;
            additionalState.av = avStatus;
            if (isIDXEnabled || isTrial) {
              if (avStatus === 1 || isTrial && avStatus === 0 && trialAccess) {
                block.loginStatus = 'loginSuccess-av1';
              } else {
                block.loginStatus = 'loginSuccess-av0';
      ...
      gaPush$1("".concat(categoryPrefix, "auth_social_success"), "form - social - ".concat(socialNamesMapping[socialName]));
    }
          ...
          target.show().ube(phoneSendName, {
            ...
            onFormLoad: function onFormLoad(_ref) {
              var alternativeCaptchaEnabled = _ref.alternativeCaptchaEnabled,
                alternativeGrecaptcha = _ref.alternativeGrecaptcha;
              initPreferredMethod({
                alternativeCaptchaEnabled: alternativeCaptchaEnabled,
                alternativeGrecaptcha: alternativeGrecaptcha,
                formData: formData
              });
              initSocialAuth({
                formKey: formKey,
    ...
    var Registration = /*#__PURE__*/function (_Block) {
      ...
      return _createClass(Registration, [{
        ...
        value: function run(key, state, cb) {
          ...
          var isIDXEnabled = !state.faceAV;
          var isValidateMode = (_state$config = state.config) === null || _state$config === void 0 || (_state$config = _state$config.coupon) === null || _state$config === void 0 ? void 0 : _state$config.validateMode;
          var formKey = this.name;
          target.show().ube(formKey, {
            isTemplatePreloaded: false,
            loadFormDataFromSession: true,
            sessionKey: state.sessionKey,
            coupon: isValidateMode && window.sessionStorage.getItem('dteCoupon'),
            data: _objectSpread2({
              phone: state.phone
            }, state.user),
            onFormLoad: function onFormLoad() {
              ...
              if (ubeSession('shouldSkipEmail') === 'false') {
                $('.ube-registration-block').hide();
                $('.ube-registration-switch').show();
                gaPush('dl_qr_2optin_process', 'view - screen_doi_fork');
                $('.skipEmailOptin').on('click', function () {
                  var skipEmailOptinValue = $(this).data('event');
                  if (skipEmailOptinValue === true) {
                    $('input[name="skipEmailOptin"]').prop('checked', true);
                    $('.ube-registration-button').click();
                    ubeSession('skipedEmailOptin', true);
                    gaPush('dl_qr_2optin_process', 'click - without_email - screen_doi_fork');
                  } else {
                    $('.ube-registration-switch').hide();
                    $('.ube-registration-block').show();
                    ubeSession('skipedEmailOptin', false);
                    gaPush('dl_qr_2optin_process', 'click - leave_email - screen_doi_fork');
                  }
                });
              } else {
                $('.ube-registration-switch').hide();
            ...
            onSubmissionSuccess: function () {
              var _onSubmissionSuccess = _asyncToGenerator(/*#__PURE__*/_regeneratorRuntime().mark(function _callee(submissionId, userId, result) {
                var _state$config2, _result$data;
                var block, additionalState, loginResult, email;
                return _regeneratorRuntime().wrap(function _callee$(_context) {
                  while (1) switch (_context.prev = _context.next) {
                    case 0:
                      window.sessionStorage.removeItem('dteCoupon');
                      block = {
                        regStatus: ''
                      };
                      additionalState = {
                      ...
                      additionalState.originalEvent = 'registration';
                      ubeCookie$1(cookies.UBE_WIDGET_REGISTER, 'Y');
                      if ((_state$config2 = state.config) !== null && _state$config2 !== void 0 && (_state$config2 = _state$config2.lead) !== null && _state$config2 !== void 0 && (_state$config2 = _state$config2.timers) !== null && _state$config2 !== void 0 && _state$config2.enabled) {
                        checkLeadTimer(state.phone);
                      }
                      _context.t0 = result.event;
                      _context.next = _context.t0 === "loginSuccess" ? 9 : _context.t0 === "alreadyVerified" ? 15 : _context.t0 === "registered" ? 22 : _context.t0 === "2optin" ? 30 : 35;
                      break;
                    case 9:
                      _context.next = 11;
                      return handleLoginSuccess({
                    ...
                    case 30:
                      ...
                      email = result === null || result === void 0 || (_result$data = result.data) === null || _result$data === void 0 || (_result$data = _result$data.data) === null || _result$data === void 0 || (_result$data = _result$data.person) === null || _result$data === void 0 ? void 0 : _result$data.email;
                      if (email) {
                        state.user = _objectSpread2(_objectSpread2({}, state.user), {}, {
                          email: email
                        });
                      }
                      cb(block, additionalState);
        ...
        value: function close(state) {
          var _state$config3;
          var email = $('.ube-form-register input[name="email"]').val();
          var isAllowRegBeforeOptin = (_state$config3 = state.config) === null || _state$config3 === void 0 || (_state$config3 = _state$config3.optin) === null || _state$config3 === void 0 ? void 0 : _state$config3.allowRegBeforeOptin;
          if (!state.withOptin || !email || isAllowRegBeforeOptin) {
            $('.ube-form-register').hide();
            ubeSession('shouldSkipEmail', null);
          }
        }
      }]);
    ...
    var sendSurvey = /*#__PURE__*/function () {
      var _ref = _asyncToGenerator(/*#__PURE__*/_regeneratorRuntime().mark(function _callee(formKey, sessionKey, data) {
        var response, result;
        return _regeneratorRuntime().wrap(function _callee$(_context) {
          while (1) switch (_context.prev = _context.next) {
            case 0:
            ...
            case 5:
              ...
              (result === null || result === void 0 ? void 0 : result.message) && showPopup(result.message);
              throw result;
            case 9:
      ...
      return function sendSurvey(_x, _x2, _x3) {
        return _ref.apply(this, arguments);
      };
    }();
    var checkRestriction = /*#__PURE__*/function () {
      var _ref2 = _asyncToGenerator(/*#__PURE__*/_regeneratorRuntime().mark(function _callee2(formKey, sessionKey) {
        var response;
        return _regeneratorRuntime().wrap(function _callee2$(_context2) {
          while (1) switch (_context2.prev = _context2.next) {
            case 0:
              _context2.next = 2;
              return fetch("".concat($.ube.host, "/api/session/").concat(formKey, "/survey/checkRestriction"), {
                method: 'GET',
                headers: {
                  'Content-Type': 'application/json',
                  'Authorization': 'Bearer ' + sessionKey
      ...
      return function checkRestriction(_x4, _x5) {
        return _ref2.apply(this, arguments);
      };
    }();
    ...
    var Survey = /*#__PURE__*/function (_Block) {
      ...
      return _createClass(Survey, [{
        ...
        value: function () {
          var _run = _asyncToGenerator(/*#__PURE__*/_regeneratorRuntime().mark(function _callee2(key, state, cb) {
            var _state$config;
            var formKey, isValidateMode, sessionKey, response, _state$config2, _response$data, hasBenefit, restrictSamePromo;
            return _regeneratorRuntime().wrap(function _callee2$(_context2) {
              while (1) switch (_context2.prev = _context2.next) {
                case 0:
                  formKey = this.name;
                  isValidateMode = (_state$config = state.config) === null || _state$config === void 0 || (_state$config = _state$config.coupon) === null || _state$config === void 0 ? void 0 : _state$config.validateMode;
                  sessionKey = state.sessionKey || ubeCookie('BITRIX_SM_ubeSessionKey');
                  toggleLoader(true);
                  _context2.next = 6;
                  return checkRestriction(formKey, sessionKey);
                case 6:
                  response = _context2.sent;
                  if (response !== null && response !== void 0 && response.data) {
                    _response$data = response.data, hasBenefit = _response$data.hasBenefit, restrictSamePromo = _response$data.restrictSamePromo;
                    if ((_state$config2 = state.config) !== null && _state$config2 !== void 0 && (_state$config2 = _state$config2.survey) !== null && _state$config2 !== void 0 && _state$config2.checkBenefitReceived && hasBenefit !== null && hasBenefit !== void 0 && hasBenefit.result) {
                      state.surveyBenefitReceived = true;
                    }
                    state.restrictSamePromo = restrictSamePromo;
                  }
                  toggleLoader(false);
                  this.options.survey(cb, state, /*#__PURE__*/function () {
                    var _ref = _asyncToGenerator(/*#__PURE__*/_regeneratorRuntime().mark(function _callee(data) {
                      ...
                      return _regeneratorRuntime().wrap(function _callee$(_context) {
                        while (1) switch (_context.prev = _context.next) {
                          case 0:
                            if (isValidateMode) {
                              data.coupon = window.sessionStorage.getItem('dteCoupon');
                            }
                            _context.next = 3;
                            return sendSurvey(formKey, state.sessionKey, data);
                          case 3:
                            result = _context.sent;
                            window.sessionStorage.removeItem('dteCoupon');
                            return _context.abrupt("return", result);
    ...
    var MAPPING = {
      ...
      emailSendNode: EmailSend,
      emailCheckNode: EmailCheck,
    ...
    function translit(word) {
      ...
      return word.toLowerCase().split('').map(function (_char) {
        var _converter$_char;
        return (_converter$_char = converter[_char]) !== null && _converter$_char !== void 0 ? _converter$_char : _char;
      }).join('');
    }

    var SECOND_SEND_TIMER = 30000;
    (function () {
      $__default["default"].fn.ubeScanpackLogin = function (name, callback, options) {
        ...
        var isTrial = options && options.trial;
        var isAuthMode = options === null || options === void 0 ? void 0 : options.authMode;
        var preferredMethodGaMapping = {
          telegram: 'tg',
          flashingCall: 'call',
          sms: 'sms'
        };
        ...
        var authType;
        var sentCounter = 0;
        var secondSendTimerPassed = false;
        var secondSendTimer;
        function incrementSentCounter() {
          sentCounter++;
          checkSendCounterVisibility();
        }
        function checkSendCounterVisibility() {
          if (sentCounter >= 2 && secondSendTimerPassed) {
            $__default["default"](".ube-resend-via-sms").show();
          } else {
            $__default["default"](".ube-resend-via-sms").hide();
          }
          if (sentCounter >= 2 && !secondSendTimer) {
            secondSendTimer = setTimeout(function () {
              secondSendTimerPassed = true;
              checkSendCounterVisibility();
            }, SECOND_SEND_TIMER);
          }
        }
        ...
        var emailSendName = name.replace("-reg", "-email-send");
        var emailCheckName = name.replace("-reg", "-email-check");
        var optinName = name.replace("-reg", "-optin");
        var phoneSendContainer = $__default["default"]('<div class="ube-form-phone-send"></div>').hide().appendTo(container);
        ...
        var emailSendContainer, emailCheckContainer;
        var isFRCookieExists = ubeCookie$1(cookies.UBE_JWT_FACE_TOKEN) || false;
        function pushDataLayer(_ref) {
          var eCategory = _ref.eCategory,
            eAction = _ref.eAction,
            eLabel = _ref.eLabel,
            userId = _ref.userId,
            userAuth = _ref.userAuth,
            codeSpace = _ref.codeSpace,
            eContent = _ref.eContent,
            eContext = _ref.eContext,
            buttonLocation = _ref.buttonLocation,
            actionGroup = _ref.actionGroup;
          var data = {
            event: 'eventLogin',
        ...
        function initPreferredMethodHandlers(formName, container, formData) {
          container.find('.ube-select-preferred-method').on('click', function () {
            container.find('.ube-preferred-method, .ube-preferred-method-shadow').addClass('ube-preferred-method_show');
          });
          container.find('.ube-preferred-method-btn').on('click', function () {
            var method = $__default["default"](this).data("method");
            formData.preferredMethod = method;
            preferredMethod = method;
            if (formName === 'phone-send') incrementSentCounter();
          });
          container.find('.ube-preferred-method-shadow, .ube-preferred-method-close, .ube-preferred-method-btn').on('click', function () {
            container.find('.ube-preferred-method-shadow, .ube-preferred-method').removeClass('ube-preferred-method_show');
          });
        }
        var handleLoginSuccess = function handleLoginSuccess(result) {
          setCookies(result.data.sessionKey, 'login');
          getInfo(result.data.sessionKey, name).done(function (response) {
            var avStatus = +response.av;
            pushDataLayer({
              ...
              codeSpace: response === null || response === void 0 ? void 0 : response.consumerCodeSpace,
        ...
        function initSocialAuth() {
          $__default["default"]('.ube-auth-social').on('click', function (e) {
            e.preventDefault();
            var socialName = $__default["default"](this).data('social');
            toggleLoader(true);
            pushDataLayer({
              eCategory: 'authorization',
              ...
              eLabel: 'sign_in',
              userAuth: '0',
              eContent: "".concat(socialName, "_id"),
              buttonLocation: 'step_1',
              actionGroup: 'interactions'
            });
            handleSocialAuth(name, socialName, onSocialAuthSuccess);
          });
        }
        function onSocialAuthSuccess(result, socialName) {
          var _result$user;
          if (result !== null && result !== void 0 && result.user) user = result.user;
          var phone = result.user.phone;
          sessionKey = result === null || result === void 0 ? void 0 : result.sessionKey;
          switch (result.event) {
            case "authorization_not_found":
              if (!phone) {
                $__default["default"](phoneSendContainer, phoneCheckContainer).find('[name="sessionKey"]').val(sessionKey);
                $__default["default"](".ube-auth-social[data-social=".concat(socialName, "]")).hide();
                toggleLoader(false);
                showPopup('Добавьте номер телефона');
                return;
              }
              if (isIDXEnabled) {
                showIDXPage(sessionKey, phone);
              } else {
                showRegistrationPage(sessionKey, phone);
              }
              eContent = "".concat(socialName, "_id");
              $__default["default"](phoneSendContainer, phoneCheckContainer).hide();
              toggleLoader(false);
              break;
            case 'authorization_success':
              ubeCookie$1(cookies.UBE_WIDGET_LOGIN, 'Y');
              ubeCookie$1(cookies.UBE_USERID, result === null || result === void 0 || (_result$user = result.user) === null || _result$user === void 0 ? void 0 : _result$user.id, sessionExpiration());
              ubeCookie$1(cookies.UBE_SESSION_KEY, result.sessionKey, sessionExpiration());
              result.data = {
                sessionKey: sessionKey
              };
              getInfo(sessionKey, name).done(function (response) {
                var avStatus = +response.av;
                pushDataLayer({
                  eCategory: 'authorization',
                  eAction: 'confirmed',
                ...
                if (isIDXEnabled) {
                  if (avStatus === 1) {
                    callback('loginSuccess', result, 'login', 1);
                  } else {
                    showAVPage(result.data.sessionKey, 'login');
                  }
                } else {
                  callback('loginSuccess', result, 'login', avStatus);
                }
                toggleLoader(false);
              });
              break;
        ...
        function initAuthSelection() {
          $__default["default"]('.ube-start-screen').show();
          $__default["default"]('.ube-auth-phone').on('click', function () {
            $__default["default"]('.ube-start-screen').hide();
            loadPhoneSend();
          });
          $__default["default"]('.ube-auth-email').on('click', function () {
            $__default["default"]('.ube-start-screen').hide();
            loadEmailSend();
          });
          initSocialAuth();
        ...
        function loadEmailSend() {
          emailSendContainer.show().ube(emailSendName, {
            sessionKey: sessionKey,
            isTemplatePreloaded: false,
            onFormLoad: function onFormLoad() {
              if (sessionKey) {
                $__default["default"](emailSendContainer).find('[name="sessionKey"]').val(sessionKey);
              }
            },
            onSubmissionSuccess: function onSubmissionSuccess(submissionId, userId, result) {
              if (result.event === 'success') {
                loadEmailCheck(result.data.sessionKey, result.data.email);
              }
            }
          });
        }
        function loadEmailCheck(sessionKey, email) {
          emailSendContainer.hide();
          emailCheckContainer.show().ube(emailCheckName, {
            isTemplatePreloaded: false,
            sessionKey: sessionKey,
            data: {
              email: email
            ...
            onSubmissionSuccess: function onSubmissionSuccess(submissionId, userId, result) {
              ...
              switch (result.event) {
                case 'verified':
                  if (isIDXEnabled) showIDXPage(result.data.sessionKey);else showRegistrationPage(result.data.sessionKey);
                  break;
                case 'loginSuccess':
                  getAvStatus(result.data.sessionKey).done(function (response) {
                    var av = response.av,
                      trialAccess = response.trialAccess;
                    var avStatus = +av;
                    if (isIDXEnabled || isDocuments || isTrial) {
        ...
        function loadPhoneSend() {
          ...
          phoneSendContainer.show().ube(phoneSendName, {
            ...
            onFormLoad: function onFormLoad() {
              ...
              initSocialAuth();
              initPreferredMethodHandlers('phone-send', phoneSendContainer, formData);
            },
            onValidationSuccess: function onValidationSuccess(data) {
              eContent = "inside_".concat(preferredMethodGaMapping[preferredMethod]);
              pushDataLayer({
                eCategory: 'authorization',
                eAction: 'button_click',
                eLabel: 'sign_in',
        ...
        function loadPhoneCheck(sessionKey, phone) {
          ...
          phoneCheckContainer.show().ube(phoneCheckName, {
            ...
            onFormLoad: function onFormLoad() {
              handleTextVisibility();
              if (preferredMethod) {
                $__default["default"](".ube-phone-check-".concat(preferredMethod)).show();
              }
              $__default["default"]('[data-ube-event="resendPhoneCode"]').on('click', function (e) {
                var eContext = translit($__default["default"](this).text());
                var resendMethodName = $__default["default"](this).attr('data-ube-preferredmethod');
                if (resendMethodName) eContent = "inside_".concat(resendMethodName);
                pushDataLayer({
                  eCategory: 'authorization',
                  eAction: 'button_click',
                  eLabel: 'sign_in',
            ...
            onValidationSuccess: function onValidationSuccess(data) {
              var btn = $__default["default"]('.ube-form-phone-check').find('button[type="submit"]');
              var eContext = translit(btn.text());
              pushDataLayer({
                eCategory: 'authorization',
                eAction: 'button_click',
                eLabel: 'sign_in',
        ...
        function showIDXPage(sessionKey, phone) {
          var _emailCheckContainer;
          $__default["default"]('.ube-start-screen').hide();
          phoneCheckContainer.hide();
          (_emailCheckContainer = emailCheckContainer) === null || _emailCheckContainer === void 0 || _emailCheckContainer.hide();
          idxContainer.appendTo(container).show().ube(idxName, {
            ...
            onSubmissionSuccess: function onSubmissionSuccess(submissionId, userId, result) {
              switch (result.event) {
                case 'success':
                  showRegistrationPage(result.data.sessionKey, phone);
              }
            }
          });
        }
        function showRegistrationPage(sessionKey, phone, result) {
          var _emailCheckContainer2;
          var formData = _objectSpread2({
            phone: phone
          }, user);
          $__default["default"]('.ube-start-screen').hide();
          phoneCheckContainer.hide();
          idxContainer.hide();
          (_emailCheckContainer2 = emailCheckContainer) === null || _emailCheckContainer2 === void 0 || _emailCheckContainer2.hide();
          registrationContainer.show().ube(name, {
            ...
            onFormLoad: function onFormLoad() {
              ...
              if (authType === 'email') {
                $__default["default"]('.ube-registration-button').addClass('ube-select-preferred-method');
                initPreferredMethodHandlers('reg', registrationContainer, formData);
              } else {
                $__default["default"]('.ube-registration-button').attr('type', 'submit');
              }
            },
            setFieldValue: function setFieldValue(key, value, common) {
              if (value) {
                if (key === 'firstName') {
                  $__default["default"]('#firstNameField').addClass('is-not-empty');
                } else if (key === 'lastName') {
                  $__default["default"]('#lastNameField').addClass('is-not-empty');
                } else if (key === 'middleName') {
                  $__default["default"]('#middleNameField').addClass('is-not-empty');
                } else if (key === 'email') {
                  $__default["default"]('#emailField').addClass('is-not-empty');
                }
                common(key, value);
              }
            },
        ...
        function showAVPage(sessionKey, originalEvent) {
          ...
          avContainer.show().ubeAV("".concat($__default["default"].ube.host, "/main/").concat(name), sessionKey, {
            ...
            onSessionError: function onSessionError(message, sessionKey) {
              showPopup("Ошибка сессии");
            }
          });
        }
        function toOptInEmail(submissionId, userId, result) {
          popupContainer.ube(optinName, {
        ...
        if (isAuthMode) {
          emailSendContainer = $__default["default"]('<div class="ube-form-email-send"></div>').hide().appendTo(container);
          emailCheckContainer = $__default["default"]('<div class="ube-form-email-check"></div>').hide().appendTo(container);
          initAuthSelection();
        } else {
          loadPhoneSend();
        }
    ...
    $__default["default"].tgLink = function (options) {
      var _options$utmSource = options.utmSource,
        ...
        _options$utmMedium = options.utmMedium,
        utmMedium = _options$utmMedium === void 0 ? getQueryParameter(window.location.search, 'utm_medium') : _options$utmMedium,
      ...
      if (utmMedium) tgLink += "&utm_medium=".concat(utmMedium);
      if (ymid) tgLink += "&ymid=".concat(ymid);
      var dteCoupon = window.sessionStorage.getItem('dteCoupon');
      if (dteCoupon) tgLink += "&coupon=".concat(dteCoupon);
      window.sessionStorage.removeItem('dteCoupon');
    ...
    function resolveHash(_x) {
      return _resolveHash.apply(this, arguments);
    }
    function _resolveHash() {
      _resolveHash = _asyncToGenerator(/*#__PURE__*/_regeneratorRuntime().mark(function _callee(name) {
        var query, redirect, utm_campaign, utm_source, utm_medium, utm_content, utm_term, url, response;
        return _regeneratorRuntime().wrap(function _callee$(_context) {
          while (1) switch (_context.prev = _context.next) {
            case 0:
              ...
              redirect = query["redirectTo"] || query["redirect"];
              ...
              ubeHostFallBack();
              url = $.ube.host + "/api/session/resolveHash?hash=" + query.hash + "&entity=" + name + "&source=" + window.location.hostname + "&ymid=" + ubeCookie$1('_ym_uid') || "";
              if (redirect) {
    ...
    var getSafeOptions = function getSafeOptions(name) {
      return $.get("".concat($.ube.host, "/api/config/safeOptions/").concat(name));
    };

    (function () {
      $.fn.ubeWidget = /*#__PURE__*/function () {
        var _ref = _asyncToGenerator(/*#__PURE__*/_regeneratorRuntime().mark(function _callee(name, options) {
          var container, faceContainer, loginContainer, timeoutEntity, showPopup, mandatoryParams, globalCtx, state, storedSafeOptions, response, userJourneyPromise, resolveHashPromise, _yield$Promise$allSet, _yield$Promise$allSet2, userJourney, hashResult, journeyResponse, _hashResult$value, _hashResult$value2, sessionKey, userData, av, _state$customUJ$find, id, data, nextBlock, key, nextBlockId, _nextBlock, skipFace, startContent, processDefaultUJ, processCustomUJ, next, startRedirect, startFaceReco, startLogin, startCustomUJ, runBlockRecursive, getNextBlock, runBlock;
          return _regeneratorRuntime().wrap(function _callee$(_context) {
            while (1) switch (_context.prev = _context.next) {
              case 0:
                runBlock = function _runBlock(key, state, cb) {
                  state.currentNodeId = state.customUJ.find(function (item) {
                    return item.key === key;
                  }).id;
                  create(key, name, options, container).runBlock(key, state, cb);
                };
                getNextBlock = function _getNextBlock(block, state) {
                  var nextBlockId;
                  var _state$customUJ$find2 = state.customUJ.find(function (item) {
                      return item.key === state.previousBlock;
                    }),
                    nextBlock = _state$customUJ$find2.nextBlock,
                    data = _state$customUJ$find2.data;
                ...
                runBlockRecursive = function _runBlockRecursive(key, state) {
                ...
                startCustomUJ = function _startCustomUJ() {
                  return runBlockRecursive(state.customUJ[0].key, state);
                };
                next = function _next(res) {
                  if (state.customUJ.length > 0) processCustomUJ(res || {});else processDefaultUJ();
                };
                processCustomUJ = function _processCustomUJ(result) {
                ...
                showPopup = function _showPopup(message) {
                  if ($.ube && $.ube.showPopup) $.ube.showPopup(message);else {
                    alert(message);
                  }
                };
                container = $(this).first();
                faceContainer = $('<div class="ube-face-container screen"></div>').appendTo(container);
                loginContainer = $('<div class="ube-login-container screen"></div>').appendTo(container);
                ...
                mandatoryParams = ['startContent', 'skuContent', 'finalContent'];
                ...
                */
                ...
                toggleLoader(true);
                _context.prev = 25;
                storedSafeOptions = sessionStorage.getItem("".concat(name, "_config"));
                if (storedSafeOptions) {
                  _context.next = 34;
                  break;
                }
                _context.next = 30;
                return getSafeOptions(name);
              case 30:
                response = _context.sent;
                if (response.config) {
                  state.config = response.config;
                  sessionStorage.setItem("".concat(name, "_config"), JSON.stringify(response.config));
                }
                _context.next = 35;
                break;
              case 34:
                state.config = JSON.parse(storedSafeOptions);
              case 35:
                _context.next = 40;
                break;
              case 37:
              ...
              case 40:
                ...
                userJourneyPromise = fetch("".concat($.ube.host, "/api/scanpack/userJourney/").concat(name));
                resolveHashPromise = resolveHash(name);
                _context.next = 45;
                return Promise.allSettled([userJourneyPromise, resolveHashPromise]);
              case 45:
                _yield$Promise$allSet = _context.sent;
                _yield$Promise$allSet2 = _slicedToArray(_yield$Promise$allSet, 2);
                userJourney = _yield$Promise$allSet2[0];
                hashResult = _yield$Promise$allSet2[1];
                _context.prev = 49;
                _context.next = 52;
                return userJourney.value.json();
              case 52:
                journeyResponse = _context.sent;
                if (!journeyResponse.journey) {
                  _context.next = 78;
                  break;
                }
                if (journeyResponse.journey.find(function (x) {
                  return x.key === 'emailOptinNode';
                })) state.withOptin = true;
                state.customUJ = journeyResponse.journey;
                $('.ube-face-container').remove();
                $('.ube-login-container').remove();
                if (!((_hashResult$value = hashResult.value) !== null && _hashResult$value !== void 0 && _hashResult$value.sessionKey && (_hashResult$value2 = hashResult.value) !== null && _hashResult$value2 !== void 0 && _hashResult$value2.userId)) {
                  _context.next = 75;
                  break;
                }
                sessionKey = hashResult.value.sessionKey;
                _context.next = 62;
                return getInfo(sessionKey, name);
              case 62:
                userData = _context.sent;
                av = +userData.av;
                _state$customUJ$find = state.customUJ.find(function (x) {
                  return x.key === 'smsPhoneCheckNode';
                }), id = _state$customUJ$find.id, data = _state$customUJ$find.data, nextBlock = _state$customUJ$find.nextBlock, key = _state$customUJ$find.key;
                state.previousBlock = id;
                state.avStatus = av;
                state.av = av;
                state.login = true;
                state.sessionKey = sessionKey;
                state.originalEvent = 'login';
                state.newUser = false;
                if (nextBlock) {
                  runBlockRecursive(key, state);
                } else {
                  nextBlockId = data.conditions.find(function (condition) {
                    return condition.value === "loginSuccess-av".concat(av);
                  }).next;
                  _nextBlock = state.customUJ.find(function (x) {
                    return x.id === nextBlockId;
                  }).key;
                  runBlockRecursive(_nextBlock, state);
                }
                _context.next = 76;
                break;
              case 75:
              ...
              case 84:
                toggleLoader(false);
                skipFace();
                startContent = function startContent() {
                  gaPush$1('dl_qr_content_start', '');
                  state.faceAV = 1;
                  state.skuShown = true;
                  ...
                  if (state.login) {
                    ...
                    if (!state.skuShown && state.ujTemplate !== 'reversed' && state.faceAV) {
                      if (state.shouldSkipFace) {
                        ...
                        gaPush$1('dl_qr_av_fr_skip', 'click - skip - form - fr');
                      }
                      startContent();
                    } else {
                      if (state.av || state.faceAV) gaPush$1('dl_qr_content_end', '');
                      startLogin();
                    }
                  } else startFaceReco();
                };
                startRedirect = function startRedirect() {
                ...
                startFaceReco = function startFaceReco() {
                  gaPush$1('dl_qr_av_fr_process', 'view - screen_fr');
                  toggleLoader(true);
                  state.face = true;
                  if (!options.classes) {
                    $('.screen').hide();
     ...
     * @param {boolean} isAllowRegBeforeOptin
     * @param {object} useCoreCRM
     * @returns {string}
     */
    var actionPathOptin = function actionPathOptin(_ref) {
      var path = _ref.path,
        configKey = _ref.configKey,
        isAllowRegBeforeOptin = _ref.isAllowRegBeforeOptin,
        useCoreCRM = _ref.useCoreCRM;
      var action_path = "/esb/".concat(path, "/submitCodes");
      if (isAllowRegBeforeOptin) {
        action_path = "/api/f2s/consumer/".concat(configKey, "/changeEmail");
      } else if (useCoreCRM) {
        action_path = "/api/session/core/registration/".concat(configKey);
      }
      return action_path;
    };
    ...
    var resendCodeCoreCRM = function resendCodeCoreCRM(configKey, options, abhash) {
      $.ajax({
        url: "".concat($.ube.host, "/api/session/core/registration/").concat(configKey),
        method: 'POST',
        contentType: 'application/json; charset=UTF-8',
        headers: {
        ...
        success: function success() {
          if (options.onResendCode) options.onResendCode();
          showPopup('Код выслан на почту');
        },
        error: function error(_error) {
          var _error$responseJSON;
          showPopup((_error$responseJSON = _error.responseJSON) === null || _error$responseJSON === void 0 ? void 0 : _error$responseJSON.message);
        }
      });
    };
    ...
    function validateYandexCaptcha(container, callback) {
      ...
      if (captchaId) {
        ...
        var categoryPrefix = getCategoryPrefix();
        var preferredMethod = ubeSession('preferredMethod');
        var phoneConfirmationType = ubeSession('phoneConfirmationType');
        var label = 'form - ya_captcha - pmi-sms-multibrand';
        if (preferredMethod === 'telegram' || phoneConfirmationType === 'telegram') {
          label = 'form - ya_captcha';
        } else if (isP1Domain()) {
          label = 'form - ya_captcha - premium-one-sms';
        }
        gaPush$1("".concat(categoryPrefix, "captcha_start"), label);
        if (widgetId == undefined || widgetId == null) {
          var sitekey = ubeSession('sitekey');
          ...
          window.smartCaptcha.subscribe(widgetId, 'challenge-visible', function () {
            return gaPush$1("".concat(categoryPrefix, "captcha_process"), label.replace('form', 'view'));
          });
          window.smartCaptcha.subscribe(widgetId, 'success', function () {
            return gaPush$1("".concat(categoryPrefix, "captcha_success"), label);
          });
     ...
     * @param {boolean} useCoreCRM
     * @returns {string}
     */
    function actionPathReg(_ref) {
      var options = _ref.options,
        path = _ref.path,
        isLeadForm = _ref.isLeadForm,
        useCoreCRM = _ref.useCoreCRM;
    ...
    var dropEmailCheckCode = /*#__PURE__*/function () {
      var _ref = _asyncToGenerator(/*#__PURE__*/_regeneratorRuntime().mark(function _callee(sessionKey) {
        var response;
        return _regeneratorRuntime().wrap(function _callee$(_context) {
          while (1) switch (_context.prev = _context.next) {
            case 0:
              _context.next = 2;
              return fetch("".concat($.ube.host, "/api/session/cabinet/dropEmailCheckCode"), {
                method: 'PATCH',
                headers: {
                  'Content-Type': 'application/json'
                },
      ...
      return function dropEmailCheckCode(_x) {
        return _ref.apply(this, arguments);
      };
    }();

    var resendCodeOnOptinAfterReg = /*#__PURE__*/function () {
      var _ref = _asyncToGenerator(/*#__PURE__*/_regeneratorRuntime().mark(function _callee(configKey, options) {
        var _options$data;
        return _regeneratorRuntime().wrap(function _callee$(_context) {
          while (1) switch (_context.prev = _context.next) {
            case 0:
              _context.prev = 0;
              _context.next = 3;
              return dropEmailCheckCode(options.sessionKey);
            case 3:
              _context.next = 5;
              return changeEmail(configKey, (_options$data = options.data) === null || _options$data === void 0 || (_options$data = _options$data.person) === null || _options$data === void 0 ? void 0 : _options$data.email, options.sessionKey);
            case 5:
              if (options.onResendCode) options.onResendCode();
              showPopup('Код выслан на почту');
              _context.next = 12;
      ...
      return function resendCodeOnOptinAfterReg(_x, _x2) {
        return _ref.apply(this, arguments);
      };
    }();
    ...
    var _handleLogin = function handleLogin(sessionKey, redirectUrl, data) {
      ...
      if ($__default["default"].ube && $__default["default"].ube.login) {
        gaPush$1(isSocialLogin ? "dl_auth_social_success" : "dl_auth_success", action, gaLabel);
    ...
    var handleSocialLoginResult = function handleSocialLoginResult(result, entity) {
      if (result && result.sessionKey) {
        ...
        if (result.socialId) {
          gaPush$1("dl_auth_social_proccess", "get_data - form - social - " + result.socialKey);
          setSocialIdCookie(result.socialId, result.socialKey, result.data);
        }
        if (result.registrationUrl) {
          gaPush$1("dl_auth_social_fail", "form - social - " + result.socialKey + " - reason: Redirect-Registration");
          window.location.href = result.registrationUrl;
        } else if (result.message) {
          gaPush$1("dl_auth_social_fail", "view - pop_up_fail_social - " + result.socialKey);
    ...
    var formalProjects = ['premiumlist_rrp', 'iqos', 'salesforce', 'qr_parliament', 'parliament_ru'];
    function isAbTestingEnabled() {
      return location.href.includes("alkoteka") && location.href.includes("qregbcp");
    }
    ...
    $__default["default"].fn.ube = function (url, options) {
      ...
      var configKey = getKeyFromUrl(((_url = url) === null || _url === void 0 || (_url = _url.data) === null || _url === void 0 ? void 0 : _url.template) || url);
      ...
      function setVariablesFromSource(src) {
        if (src && src.data && src.data.form) {
          ...
          options.loadFormDataFromSession = false;
          var d = src.data;
          if ((_options = options) !== null && _options !== void 0 && (_options = _options.data) !== null && _options !== void 0 && _options.preferredMethod) d.preferredMethod = options.data.preferredMethod;
      ...
      function initializeFormByConfig(_x) {
        return _initializeFormByConfig.apply(this, arguments);
      }
      function _initializeFormByConfig() {
        _initializeFormByConfig = _asyncToGenerator(/*#__PURE__*/_regeneratorRuntime().mark(function _callee2(json) {
          var name, isWizard, isLeadForm, formProperties, captchaType, captchaBypass, components, validationState, isQRForm, isMBEPForm, categoryPrefix, remainingAttemptsCount, phoneConfirmationType, configFromSession, res, factory, fieldMap, fieldNames, hasField, getField, setFieldValue, fieldNamesByTag, getFieldValue, resetOptinAfterSubmission, context, fieldNamesByPart, appendChildrenKeys, ajaxLookupGender, wto, previousLookupResult, autolookupGender, resetFieldValidation, setFieldValid, setFieldInvalid, validateComponents, validateComponent, lastValidatedValue, validateField, getFieldObject, getFormData, handleVisibility, handleFormVisibility, handleResponseErrors, isVisible, toggleVisible, validateAllFields, isLoading, toggleLoader, showPopup, handle_form, template_url, isTemplatePreloaded;
          return _regeneratorRuntime().wrap(function _callee2$(_context2) {
            while (1) switch (_context2.prev = _context2.next) {
              case 0:
                showPopup = function _showPopup(message) {
                ...
                toggleLoader = function _toggleLoader(visible) {
                  isLoading = visible;
                  console.log("UBE Toggle loader " + visible);
                  if (options.toggleLoader) options.toggleLoader(visible);
                };
                validateAllFields = function _validateAllFields(handleValid, handleInvalid, fieldNames) {
                ...
                toggleVisible = function _toggleVisible(key, visible) {
                  ...
                  if (visible) {
                    ...
                    if (!wasVisible && !events.initIdx && component.tags && component.tags.includes("initIdx")) {
                      gaPush$1(categoryPrefix + "av_process", "click - button - idx");
                      events.initIdx = true;
                      toggleLoader(false);
                    }
                    ...
                    if (!wasVisible && !events.sendMegafonCode && component.tags && component.tags.indexOf("sendMegafonCode") >= 0) {
                      gaPush$1("AV", "Initialize", "megafon");
                ...
                isVisible = function _isVisible(key, formData) {
                  var component = fieldMap[key];
                ...
                handleResponseErrors = function _handleResponseErrors(response) {
                  var _response$errors, _response$message2;
                  ...
                  } else if (((_response$message2 = response.message) === null || _response$message2 === void 0 ? void 0 : _response$message2.length) > 0) {
                    showPopup(response.message);
                  }
                  toggleLoader(false);
                };
                handleFormVisibility = function _handleFormVisibility(exceptKey) {
                  var formData = getFormData();
                  fieldNames.forEach(function (key) {
                    if (key != exceptKey) handleVisibility(key, formData);
                  });
                };
                handleVisibility = function _handleVisibility(key, formData) {
                  return toggleVisible(key, isVisible(key, formData));
                };
                getFormData = function _getFormData(part) {
                  var formData = {};
                  ...
                  if ($__default["default"].ube.hash != null && isAbTestingEnabled()) {
                    formData.abhash = $__default["default"].ube.hash;
                  }
                  if (!isLeadForm) {
                ...
                getFieldObject = function _getFieldObject(key) {
                  return getField(key).data('data-object') || getFieldValue(key);
                };
                validateField = function _validateField(key, value, handleValid, handleInvalid, handleError, formData) {
                  var component = fieldMap[key];
                  ...
                  function ajaxValidate(path, failMessage, handleValidFun) {
                    ajaxValidateCall(path, function (valid, response) {
                      if (typeof failMessage === 'function') {
                        failMessage = failMessage(response);
                      }
                      if (valid) {
                        if (handleValidFun) {
                          handleValidFun(key, value);
                        } else {
                  ...
                  var requiredByGroupA = hasTag("require_by_group_a") && $__default["default"].ube.abGroup === "A";
                  var requiredByGroupB = hasTag("require_by_group_b") && $__default["default"].ube.abGroup === "B";
                  var isGroupRequired = (requiredByGroupA || requiredByGroupB) && isAbTestingEnabled();
                  var emailErrorForGroupRequired = 'Вы можете зарегистрироваться без указания емейла в боте <a href="https://max.ru/id7710298176_13_bot?start=qreg_Alkoteka" target="_blank">MAX</a> или в <a href="https://iqosmeetingbot.cleverbots.ru/redirect_to_bot/RegMe_Q?start=QREG_Alkoteka" target="_blank">Telegram</a>';
                  var customMessage = rules.customMessage && rules.customMessage != "" ? rules.customMessage : "Пожалуйста, укажите " + component.label;
                  if (isGroupRequired && (value || "").length == 0) {
                    if (key === 'email') {
                      customMessage = emailErrorForGroupRequired;
                    }
                  ...
                  if (component.type == 'email') {
                    ...
                    return ajaxValidate("/validate/email?email=" + value + "&entity=" + name + "&optin=" + hasTag("optin"), function (response) {
                      if (isGroupRequired) {
                        return emailErrorForGroupRequired;
                      } else if (response.message === "Foreign Email") {
                        var _formalProjects$inclu;
                        var pronoun = (_formalProjects$inclu = formalProjects.includes) !== null && _formalProjects$inclu !== void 0 && _formalProjects$inclu.call(formalProjects, response.esbProject) ? "используйте" : "используй";
                        return "\u041F\u043E\u0436\u0430\u043B\u0443\u0439\u0441\u0442\u0430, ".concat(pronoun, " \u043F\u043E\u0447\u0442\u0443 \u0441 \u0440\u043E\u0441\u0441\u0438\u0439\u0441\u043A\u0438\u043C \u0434\u043E\u043C\u0435\u043D\u043E\u043C. \u041D\u0430\u043F\u0440\u0438\u043C\u0435\u0440, @mail.ru \u0438\u043B\u0438 @yandex.ru.");
                      }
                      return component.validate.customMessage || "Некорректный или не существующий email адрес";
                    }, function () {
                      ...
                      var failMessage;
                      if (uniqueError && uniqueError !== "") {
                        failMessage = uniqueError;
                      } else if (isGroupRequired) {
                        failMessage = emailErrorForGroupRequired;
                      } else {
                        failMessage = "E-Mail адрес уже был зарегистрирован ранее";
                      }
                      if (hasTag("validateNotExists")) {
                        ajaxValidate("/validate/emailNotExists?email=" + value + userIdPart, failMessage);
                      } else {
                  ...
                  if (component.type == 'phoneNumber') {
                    ...
                    return ajaxValidate("/validate/phone?number=" + value + "&entity=" + name, function (response) {
                      var _response$message;
                      if (response !== null && response !== void 0 && (_response$message = response.message) !== null && _response$message !== void 0 && _response$message.includes('Что-то пошло не так')) return response.message;else return component.validate.customMessage || "Введите существующий номер телефона";
                    }, function () {
                      var userId = hasField("userId") ? getFieldValue("userId") : null;
                      var userIdPart = userId ? "&userId=" + userId : "";
                      var uniqueError = component.uniqueError;
                  ...
                  if (value && value.length > 0 && key === 'coupon') {
                    return ajaxValidateCall("/validate/coupon?code=" + value + "&entity=" + name, function (valid, response) {
                      if (valid) handleValid(key, value);else if (options.validateValue) options.validateValue(key, value, handleValid, handleInvalid);else handleInvalid(key, value, response.message || "Введите существующий код");
                    });
                  }
                  ...
                  if (value && value.length > 0 && hasTag("validateBonusCard")) return ajaxValidateCall("/validate/bonusCard?value=" + value + "&entity=" + name, function (valid, response) {
                    if (valid) handleValid(key, value);else handleInvalid(key, value, response.message);
                  });
                  handleValidFromOption(key, value);
                };
                validateComponent = function _validateComponent(key, handleValid, handleInvalid, onlyToValid, event) {
                  var value = getFieldValue(key);
                ...
                validateComponents = function _validateComponents(keys, tags, handleAllValid, handleSomeInvalid) {
                ...
                setFieldInvalid = function _setFieldInvalid(key, value, description) {
                  ...
                  if (key && gaFieldSet[key] !== false) {
                    ...
                    if (/-phone-send$/.test(name)) {
                      ...
                      var phoneTypeActions = {
                        telegram: "number - form - phone_telegram_push - reason: ".concat(description),
                        sms: "number - form - phone_sms_code - reason: ".concat(description),
                        flashingCall: "number - form - phone_call_code - reason: ".concat(description)
                      };
                      var action;
                      ...
                      if (remainingAttemptsCount) {
                        updatePhoneTypeActions(_phoneTypeActions, " - attempts_left: ".concat(remainingAttemptsCount, " - reason: ").concat(description));
                      } else {
                        updatePhoneTypeActions(_phoneTypeActions, " - reason: ".concat(description));
                      }
                      if (isMBEPForm && !isP1Domain()) {
                        _action = "form - phone_code - reason: ".concat(description);
                      } else if (preferredMethod) {
                        _action = _phoneTypeActions[preferredMethod];
                      } else if (phoneConfirmationType) {
                        _action = _phoneTypeActions[phoneConfirmationType];
                      }
                      gaPush$1(_category, _action);
                      ...
                      if (isMBEPForm && $__default["default"]('.reg-form_step_0').is(':visible')) {
                        gaPush$1('dl_av_idx_registration_fail', "form - idx_reg - ".concat(key, " - reason: ").concat(description));
                      } else {
                        gaPush$1(_category2, "form - registration - ".concat(key, " - reason: ").concat(description));
                      }
                    } else if (/-optin$/.test(name)) {
                      gaPush$1(categoryPrefix + '2optin_fail', "form - 2optin - reason: ".concat(description));
                    } else if (/-idx$/.test(name)) {
                      gaPush$1(categoryPrefix + 'av_idx_registration_fail', "form - idx_reg - ".concat(key, " - reason: ").concat(description));
                    } else if (/remind/.test(name)) {
                      gaPush$1(categoryPrefix + "auth_resetPassword_fail", "form - resetPassword - reason: ".concat(description));
                    } else if (/-login$/.test(name)) {
                      gaPush$1(categoryPrefix + "auth_password_fail", "form - password - reason: ".concat(description));
                    }
                  }
                  (custom || common)(key, value, description, common);
                };
                setFieldValid = function _setFieldValid(key, value) {
                  var common = function common(key, value) {
                ...
                resetFieldValidation = function _resetFieldValidation(key) {
                  container.find(".ube-validation-set-class-" + key).each(function () {
                ...
                autolookupGender = function _autolookupGender() {
                ...
                ajaxLookupGender = function _ajaxLookupGender(firstName, lastName, handler) {
                  $__default["default"].ajax({
                ...
                fieldNamesByPart = function _fieldNamesByPart(part) {
                ...
                resetOptinAfterSubmission = function _resetOptinAfterSubmi() {
                ...
                name = json.path;
                isWizard = json.display == "wizard";
                isLeadForm = json.display == "leadForm";
                formProperties = json.properties || {};
                captchaType = formProperties.captchaType || 'google';
                captchaBypass = json.captchaBypass;
                components = flattenComponents(json.components);
                validationState = {};
                isQRForm = isQRDomain();
                isMBEPForm = isMbepDomain() && !window.location.host.includes('qr');
                categoryPrefix = getCategoryPrefix();
                configFromSession = JSON.parse(ubeSession("".concat(configKey, "_config")));
                if (!safeOptionData) {
                  _context2.next = 36;
                  break;
                }
                ...
                break;
              case 36:
                if (!(!safeOptionData && configFromSession)) {
                  _context2.next = 41;
                  break;
                }
              ...
              case 41:
                _context2.next = 43;
                return getSafeOptions(configKey);
              case 43:
                res = _context2.sent;
                options.config = res === null || res === void 0 ? void 0 : res.config;
                safeOptionData = res === null || res === void 0 ? void 0 : res.config;
              case 46:
                 ...
                 * @type {Factory}
                 */
                factory = new Factory(container, components, options);
                fieldMap = factory.fieldMap, fieldNames = factory.fieldNames, hasField = factory.hasField, getField = factory.getField, setFieldValue = factory.setFieldValue, fieldNamesByTag = factory.fieldNamesByTag;
                getFieldValue = function getFieldValue(key) {
                  return factory.getFieldValue(key, initialValues[key]);
                };
                context = new Context();
                appendChildrenKeys = function appendChildrenKeys(key) {
                  var component = fieldMap[key];
                ...
                lastValidatedValue = {};
                isLoading = false;
                handle_form = function handle_form() {
                  var _options$config, _json$properties, _options$config2;
                  var form = container.find("form");
                  var path = json.path;
                  var action_path;
                  var ga_name;
                  var isCouponValidateMode = (_options$config = options.config) === null || _options$config === void 0 || (_options$config = _options$config.coupon) === null || _options$config === void 0 ? void 0 : _options$config.validateMode;
                  phoneConfirmationType = (json === null || json === void 0 || (_json$properties = json.properties) === null || _json$properties === void 0 ? void 0 : _json$properties.confirmationType) || ubeSession('phoneConfirmationType');
                  ...
                  var isAllowRegBeforeOptin = (_options$config2 = options.config) === null || _options$config2 === void 0 || (_options$config2 = _options$config2.optin) === null || _options$config2 === void 0 ? void 0 : _options$config2.allowRegBeforeOptin;
                  factory.autoCreateAbsentFields();
                  var timer = new Timer();
                  if (/-reg$/.test(path)) {
                    if (formProperties !== null && formProperties !== void 0 && formProperties.useCoreCRM) {
                      useCoreCRM = true;
                    }
                    action_path = actionPathReg({
                      options: options,
                      path: path,
                      isLeadForm: isLeadForm,
                      useCoreCRM: useCoreCRM
                    });
                    var category = isQRForm ? 'dl_qr_registration_start' : 'dl_auth_registration_start';
                    if (!isMBEPForm) {
                      if (ubeSession('shouldSkipEmail') === 'true') {
                        gaPush$1(category, 'form - registration - with_email');
                      } else if (ubeSession('shouldSkipEmail') === 'false') {
                        gaPush$1(category, 'form - registration - wo_email');
                      }
                      gaPush$1(category, "form - registration");
                    }
                    if (isMBEPForm) {
                      gaPushObject({
                        event: 'dl-pageview',
                    ...
                    if (isQRForm) gaPush$1("qr_reg", isFRPassed ? "fr - success" : "fr - fail - screen_2", promoCampaignName);
                    ga_name = "Registration";
                  } else if (/optin$/.test(path)) {
                    action_path = actionPathOptin({
                      path: path,
                      configKey: configKey,
                      isAllowRegBeforeOptin: isAllowRegBeforeOptin,
                      useCoreCRM: useCoreCRM
                    });
                    var _category3 = categoryPrefix + '2optin_start';
                    var action = 'form - 2optin';
                    gaSkipEmailOptin(_category3, action);
                    gaPush$1(_category3, action);
                    ga_name = "2OPTIN";
                  } else if (/av$/.test(path)) {
                    action_path = "/esb/" + path + "/submission";
                    ga_name = "AV";
                    if (hasField("faceMethod")) {
                      gaPush$1(categoryPrefix + 'av_process', 'view - screen_av_fork');
                    } else if (hasField("richcallMethod")) {
                      options.richcall && options.richcall();
                    } else {
                      gaPush$1(categoryPrefix + "av_idx_start", "form - idx");
                    }
                  } else if (/login$/.test(path)) {
                    action_path = "/esb/" + path + "/submission";
                    ga_name = "Login";
                    gaPush$1(categoryPrefix + "auth_password_start", "form - password");
                    if (isMBEPForm) {
                      gaPushObject({
                        event: 'dl-pageview',
                        pageURL: window.location.href,
                    ...
                    action_path = "/esb/" + path + "/submission";
                    ga_name = "LoginRemind";
                    gaPush$1(categoryPrefix + "auth_resetPassword_start", "form - resetPassword");
                    if (isMBEPForm) {
                      gaPushObject({
                        event: 'dl-pageview',
                        pageURL: window.location.href,
                    ...
                    var preferredMethod = ubeSession('preferredMethod');
                    var phoneTypeActions = {
                      telegram: 'input - telegram_push - form - phone_telegram_push',
                      sms: 'input - sms_code - form - phone_sms_code',
                      flashingCall: 'input - call_code - form - phone_call_code'
                    };
                    if (!isMBEPForm && !isQRForm) {
                      var _action2;
                      $__default["default"]('input[name=submittedPhoneCode]').one('input', function () {
                        if (preferredMethod) {
                          _action2 = phoneTypeActions[preferredMethod];
                        } else if (phoneConfirmationType) {
                          _action2 = phoneTypeActions[phoneConfirmationType];
                        }
                        if (remainingAttemptsCount) {
                          updatePhoneTypeActions(phoneTypeActions, " - attempts_left: ".concat(remainingAttemptsCount));
                        }
                        gaPush$1('dl_auth_phone_process', _action2);
                      });
                    }
                    ...
                    if (isMBEPForm) {
                      ...
                      gaPush$1("dl_auth_phone_start", "form - phone");
                    } else if (isQRForm) {
                      gaPush$1("qr_phone", isFRPassed ? "fr - success" : "fr - fail", promoCampaignName);
                    } else {
                      gaPush$1('dl_auth_phone_process', 'view - screen_phone_number');
                    }
                  } else if (/-email-send$/.test(path)) {
                    action_path = "/esb/" + path + "/submission";
                  } else if (/-email-check$/.test(path)) {
                    action_path = "/esb/" + path + "/submission";
                  } else if (/-idx$/.test(path)) {
                    action_path = "/esb/" + path + "/submission";
                    ga_name = "IDXVerification";
                    gaPush$1(categoryPrefix + "av_idx_registration_start", "form - idx_reg");
                    if (isQRForm) gaPush$1("qr_reg", "fr - fail - screen_1", promoCampaignName);
                  } else if (/-coupon$/.test(path)) {
                    var key = path.replace('-coupon', '-reg');
                  ...
                  function ajaxSubmitFormInternal(data, isSearchOnly) {
                    ...
                    var isCoreCRM = useCoreCRM && requestPath.includes('/api/session/core/registration/');
                    ...
                    if (isCouponValidateMode && requestPath.includes('/api/validate/coupon')) {
                      var couponCode = $__default["default"]('input[name="couponCode"]').val();
                      method = 'GET';
                      requestPath += "&code=" + couponCode;
                      payload = undefined;
                    }
                    form.attr("action", host + requestPath);
                    if ((_options2 = options) !== null && _options2 !== void 0 && (_options2 = _options2.data) !== null && _options2 !== void 0 && _options2.preferredMethod) {
                    ...
                    if (ga_name === 'AV' && ubeSession('skipedEmailOptin') === 'false') {
                      payload.data.skipEmailOptin = false;
                    }
                    if (isCoreCRM || dteCoupon) {
                      payload = data.data;
                    }
                    if (isAllowRegBeforeOptin && requestPath.includes('/changeEmail')) {
                      var _options$data;
                      payload.email = (_options$data = options.data) === null || _options$data === void 0 || (_options$data = _options$data.person) === null || _options$data === void 0 ? void 0 : _options$data.email;
                      payload.emailConfirmationCode = +data.data.emailConfirmationCode;
                    }
                    ...
                    $__default["default"].ajax({
                      ...
                      error: function error(xhr, resp, text) {
                        ...
                        if (/-reg$/.test(path)) {
                          ...
                          gaPush$1(_category4, "form - registration - reason: technical");
                          if (isMBEPForm) {
                            if ($__default["default"]('.reg-form_step_0').is(':visible')) gaPush$1('dl_av_idx_registration_fail', "form - idx_reg - reason: technical");
                            gaPushObject({
                              event: 'dl-pageview',
                              pageURL: window.location.href,
                              pageType: 'auth_nonBrand'
                          ...
                          ga_name = "Registration";
                          if (useCoreCRM) {
                            return handleResponseErrors(xhr.responseJSON);
                          }
                        } else if (/optin$/.test(path)) {
                          action_path = actionPathOptin({
                            path: path,
                          ...
                          gaPush$1(categoryPrefix + "2optin_fail", "form - 2optin - reason: technical");
                          ga_name = "2OPTIN";
                          if (isAllowRegBeforeOptin || useCoreCRM) {
                            return handleResponseErrors(xhr.responseJSON);
                          }
                        } else if (/av$/.test(path)) {
                          action_path = "/esb/" + path + "/submission";
                          gaPush$1(categoryPrefix + "av_idx_fail", "form - idx - reason: technical");
                          ga_name = "AV";
                        } else if (/-phone-check$/.test(path)) {
                          action_path = "/esb/" + path + "/submission";
                          ga_name = "PhoneCodeVerification";
                          var _category5 = isQRForm ? 'dl_qr_phone_fail' : 'dl_auth_phone_fail';
                          var _phoneTypeActions2 = {
                          ...
                          var _action3;
                          if (isMBEPForm && !isP1Domain()) {
                            _action3 = 'form - phone_number - reason: technical';
                          } else if (preferredMethod) {
                            _action3 = _phoneTypeActions2[preferredMethod];
                          } else if (phoneConfirmationType) {
                            _action3 = _phoneTypeActions2[phoneConfirmationType];
                          }
                          gaPush$1(_category5, _action3);
                        } else if (/-phone-send$/.test(path)) {
                          action_path = "/esb/" + path + "/submission";
                          var _category6 = isQRForm ? 'dl_qr_phone_fail' : 'dl_auth_phone_fail';
                          var _phoneTypeActions3 = {
                            telegram: 'telegram_push - form - phone_telegram_push',
                            sms: 'sms_code - form - phone_sms_code',
                            flashingCall: 'call_code - form - phone_call_code'
                          };
                          var _action4;
                          if (remainingAttemptsCount) {
                            updatePhoneTypeActions(_phoneTypeActions3, " - attempts_left: ".concat(remainingAttemptsCount));
                          } else {
                            updatePhoneTypeActions(_phoneTypeActions3, ' - reason: technical');
                          }
                          if (isMBEPForm && !isP1Domain()) {
                            _action4 = 'form - phone_code - reason: technical';
                          } else if (preferredMethod) {
                            _action4 = _phoneTypeActions3[preferredMethod];
                          } else if (phoneConfirmationType) {
                            _action4 = _phoneTypeActions3[phoneConfirmationType];
                          }
                          gaPush$1(_category6, _action4);
                        } else if (/-idx$/.test(path)) {
                          action_path = "/esb/" + path + "/submission";
                          ga_name = "IDXVerification";
                          gaPush$1(categoryPrefix + "av_idx_registration_fail", "form - idx_reg - reason: technical");
                          if (isQRForm) gaPush$1("qr_reg", "fr - fail - screen_1", promoCampaignName);
                        } else if (/-coupon$/.test(path)) {
                          return handleResponseErrors(xhr.responseJSON);
                        }
                      ...
                      function handleResult(result) {
                        ...
                        function gaCheck(error) {
                          if (ga_name === 'PhoneCodeVerification') gaPush$1(categoryPrefix + 'auth_fail', 'method: phone - invalid-password', 'Введен некорректный логин (email или моб. телефон)');else if (ga_name === 'Login') gaPush$1(categoryPrefix + 'auth_password_fail', 'form - password - reason: ' + error);else if (ga_name === 'LoginRemind') gaPush$1(categoryPrefix + 'auth_resetPassword_fail', 'form - resetPassword - reason: ' + error);
                        }
                      ...
                      function submissionSuccess(result) {
                        ...
                        if (name.includes('phone-send')) {
                          ...
                          if (isMBEPForm && !isP1Domain()) {
                            ...
                            _action5 = _phoneTypeActions4[preferredMethod];
                          } else if (phoneConfirmationType) {
                            _action5 = _phoneTypeActions4[phoneConfirmationType];
                          }
                          gaPush$1(_category7, _action5);
                          ...
                          var status = result.event === 'loginSuccess' ? 'in_base' : 'not_in_base';
                          var _phoneTypeActions5 = {
                            telegram: "form - phone_telegram_push - (".concat(status, ")"),
                            sms: "form - phone_sms_code - (".concat(status, ")"),
                            flashingCall: "form - phone_call_code - (".concat(status, ")")
                          };
                          var _action6;
                          if (remainingAttemptsCount) {
                            updatePhoneTypeActions(_phoneTypeActions5, " - attempts_left: ".concat(remainingAttemptsCount));
                          }
                          if (isMBEPForm && !isP1Domain()) {
                            _action6 = "form - phone - (".concat(status, ")");
                          } else if (preferredMethod) {
                            _action6 = _phoneTypeActions5[preferredMethod];
                          } else if (phoneConfirmationType) {
                            _action6 = _phoneTypeActions5[phoneConfirmationType];
                          }
                          gaPush$1(_category8, _action6);
                          ...
                          var _action7 = 'form - registration';
                          gaSkipEmailOptin(_category9, _action7);
                          gaPush$1(_category9, _action7);
                          ...
                          var _category10 = categoryPrefix + '2optin_success';
                          var _action8 = 'form - 2optin';
                          gaSkipEmailOptin(_category10, _action8);
                          gaPush$1(_category10, _action8);
                        } else if (name.includes('-av')) {
                          if (!hasField('faceMethod') || data.data.method === 'document') {
                            gaPush$1(categoryPrefix + 'av_idx_success', 'form - idx - document: ' + payload.data.documentType);
                          }
                        } else if (name.includes('-idx')) {
                          gaPush$1(categoryPrefix + 'av_idx_registration_success', 'form - idx_reg');
                        }
                        if (ga_name === 'LoginRemind') gaPush$1(categoryPrefix + 'auth_resetPassword_success', 'form - resetPassword');else if (ga_name === 'Login') {
                          var _action9;
                          ...
                          gaPush$1(categoryPrefix + 'auth_password_success', "form - password - ".concat(_action9));
                        }
                  ...
                  function onFormSubmit(part) {
                    ...
                    validateAllFields(function () {
                      ...
                      validationState = {};
                      if (/-reg$/.test(path)) {
                        var _category11 = isQRForm ? 'dl_qr_registration_process' : 'dl_auth_registration_process';
                        gaPush$1(_category11, 'send - form - registration');
                      } else if (/optin$/.test(path)) {
                        actionPathOptin({
                          path: path,
                          configKey: configKey,
                        ...
                        if (isAllowRegBeforeOptin) {
                          action_path = "/api/f2s/consumer/" + configKey + "/changeEmail";
                        }
                        var _category12 = categoryPrefix + '2optin_process';
                        var _action10 = 'send - form - 2optin';
                        gaSkipEmailOptin(_category12, _action10);
                        gaPush$1(_category12, _action10);
                        ga_name = "2OPTIN";
                      } else if (/av$/.test(path)) {
                        action_path = "/esb/" + path + "/submission";
                        ga_name = "AV";
                        gaPush$1(categoryPrefix + 'av_idx_process', "send - form - idx - document: ".concat(data.data.documentType));
                      } else if (/login$/.test(path)) {
                        action_path = "/esb/" + path + "/submission";
                        ga_name = "Login";
                        gaPush$1(categoryPrefix + "auth_password_process", "send - form - password");
                        if (isMBEPForm) {
                          gaPushObject({
                            event: 'dl-pageview',
                            pageURL: window.location.href,
                        ...
                        action_path = "/esb/" + path + "/submission";
                        ga_name = "LoginRemind";
                        gaPush$1(categoryPrefix + "auth_resetPassword_process", "send - form - resetPassword");
                        if (isMBEPForm) {
                          gaPushObject({
                            event: 'dl-pageview',
                            pageURL: window.location.href,
                        ...
                        var _category13 = isQRForm ? 'dl_qr_phone_process' : 'dl_auth_phone_process';
                        var _phoneTypeActions6 = {
                        ...
                        var _action11;
                        if (remainingAttemptsCount) {
                          updatePhoneTypeActions(_phoneTypeActions6, " - attempts_left: ".concat(remainingAttemptsCount));
                        }
                        if (isMBEPForm && !isP1Domain()) {
                          _action11 = 'send - form - phone_code';
                        } else if (preferredMethod) {
                          _action11 = _phoneTypeActions6[preferredMethod];
                        } else if (phoneConfirmationType) {
                          _action11 = _phoneTypeActions6[phoneConfirmationType];
                        }
                        gaPush$1(_category13, _action11);
                      } else if (/-phone-send$/.test(path)) {
                        var _category14 = isQRForm ? 'dl_qr_phone_process' : 'dl_auth_phone_process';
                        var _phoneTypeActions7 = {
                        ...
                        var _action12;
                        if (isMBEPForm && !isP1Domain()) {
                          _action12 = 'send - form - phone_number';
                        } else if (preferredMethod) {
                          _action12 = _phoneTypeActions7[preferredMethod];
                        } else if (phoneConfirmationType) {
                          var _category15 = isQRDomain() ? 'dl_qr_' : 'dl_auth_';
                          _action12 = _phoneTypeActions7[phoneConfirmationType];
                          if (phoneConfirmationType === 'telegram') {
                            gaPush$1("".concat(_category15, "phone_start"), "form - phone_telegram_push");
                          } else if (phoneConfirmationType === 'sms') {
                            gaPush$1("".concat(_category15, "phone_start"), "form - phone_sms_code");
                          } else if (phoneConfirmationType === 'flashingCall') {
                            gaPush$1("".concat(_category15, "phone_start"), "form - phone_call_code");
                          }
                        }
                        gaPush$1(_category14, _action12);
                      } else if (/-idx$/.test(path)) {
                        action_path = "/esb/" + path + "/submission";
                        gaPush$1(categoryPrefix + "av_idx_registration_process", "send - form - idx_reg");
                      }
                      ...
                      if (options && options.onFieldInvalid) {
                        for (var _i = 0, _Object$entries = Object.entries(invalids); _i < _Object$entries.length; _i++) {
                          ...
                          if (validationState[_key] !== invalids[_key]) options.onFieldInvalid(_key, value);
                        }
                  ...
                  function initializeFaceCapture() {
                    var _options$config3, _options$config4;
                    var renderContainer = container.find(".ube-camera-container");
                    var renderTarget = container.find(".ube-camera-render");
                    var captureButton = container.find(".ube-camera-capture");
                    var fallbackTarget = container.find(".ube-camera-fallback");
                    var vendorsList = (_options$config3 = options.config) === null || _options$config3 === void 0 || (_options$config3 = _options$config3.av) === null || _options$config3 === void 0 || (_options$config3 = _options$config3.face) === null || _options$config3 === void 0 ? void 0 : _options$config3.vendorsList;
                    var faceState = {
                      ...
                      tryCount: 1,
                      options: options,
                      priorityFaceVendor: ((_options$config4 = options.config) === null || _options$config4 === void 0 || (_options$config4 = _options$config4.av) === null || _options$config4 === void 0 || (_options$config4 = _options$config4.face) === null || _options$config4 === void 0 ? void 0 : _options$config4.primaryFaceVendor) || 'idx',
                      onSuccess: onSuccess,
                      onError: onError
                    };
                    ...
                    function renderCameraCapture() {
                      if (isNoCameraStream) {
                        return $__default["default"].ube.host.includes('test') ? renderFileUpload() : switchDocumentMethod();
                      }
                      fallbackTarget.hide();
                      renderContainer.addClass("ube-camera-option-capture").removeClass("ube-camera-option-upload");
                      $__default["default"]('.ube-camera-capture').hide();
                      var onFaceSuccess = function onFaceSuccess(_ref) {
                        ...
                        dummyImg.src = img;
                        gaPush$1(categoryPrefix + "av_fr_process", "send - form - fr_auto");
                      };
                      var onFaceError = function onFaceError(err) {
                        var errLabel = err.message || err || 'Unknown error';
                        console.log('Face capture error:', errLabel);
                        gaPush$1(categoryPrefix + "av_fr_process", "start - form - fr_upload - reason: ".concat(errLabel));
                        $__default["default"].ube.host.includes('test') ? renderFileUpload() : switchDocumentMethod();
                      };
                      var onReadyForCaptureFace = function onReadyForCaptureFace() {
                        gaPush$1(categoryPrefix + "av_fr_process", "start - form - fr_auto");
                      };
                      var props = {
                    ...
                    function renderFileUpload() {
                      ...
                      dummyFile.off("change").change(function () {
                        ...
                        gaPush$1(categoryPrefix + "av_fr_process", "send - form - fr_upload");
                      });
                      captureButton.add(".ube-camera-option-upload").off("click").click(function (e) {
                        console.log("Capture upload clicked");
                        e.preventDefault();
                  ...
                  fieldNames.forEach(function (key) {
                    ...
                    $__default["default"](".ube-value-update-on-click-" + key).click(function () {
                      ...
                      if (option === 'face') {
                        gaPush$1(categoryPrefix + 'av_process', 'click - button - fr');
                        gaPush$1(categoryPrefix + "av_fr_start", "form - fr");
                  ...
                  container.off("click", ".ube-trigger-event").on("click", ".ube-trigger-event", /*#__PURE__*/function () {
                    var _ref2 = _asyncToGenerator(/*#__PURE__*/_regeneratorRuntime().mark(function _callee(e) {
                      var event, repeat, cabinetUrl, preferredMethod, category, action, _phoneTypeActions8, data, formData, _phoneTypeActions9, key, finalUrl;
                      return _regeneratorRuntime().wrap(function _callee$(_context) {
                        while (1) switch (_context.prev = _context.next) {
                          case 0:
                            e.preventDefault();
                          ...
                          case 3:
                            event = $__default["default"](this).attr("data-ube-event");
                            repeat = $__default["default"](this).attr("data-ube-repeat");
                            cabinetUrl = $__default["default"](this).attr("data-cabinet-url") || "/personal/age-verification";
                            preferredMethod = $__default["default"](this).attr("data-ube-preferredMethod") || ubeSession('preferredMethod');
                            category = isQRForm ? 'dl_qr_' : 'dl_auth_';
                            if (event.includes('close') && /-phone-check$/.test(path)) {
                              _phoneTypeActions8 = {
                              ...
                              if (preferredMethod) {
                                action = _phoneTypeActions8[preferredMethod];
                              } else if (phoneConfirmationType) {
                                action = _phoneTypeActions8[phoneConfirmationType];
                              }
                              gaPush$1("".concat(category, "phone_process"), action);
                            } else if (name.endsWith('optin')) {
                              gaPush$1(categoryPrefix + "2optin_process", "click - button - back");
                            } else if (name.includes('cabinet')) {
                              gaPush$1('dl_lk_profile', 'confirm - ' + event.split('-').slice(-1).join());
                            }
                            ...
                            if (/-phone-check$/.test(path)) {
                              ...
                              if (remainingAttemptsCount) {
                                updatePhoneTypeActions(_phoneTypeActions9, " - attempts_left: ".concat(remainingAttemptsCount));
                              }
                              if (preferredMethod) {
                                action = _phoneTypeActions9[preferredMethod];
                              } else if (phoneConfirmationType) {
                                action = _phoneTypeActions9[phoneConfirmationType];
                              }
                              gaPush$1("".concat(category, "phone_process"), action);
                            }
                            if (useCoreCRM) {
                              resendCodeCoreCRM(configKey, options, $__default["default"].ube.hash);
                            } else if (isAllowRegBeforeOptin && action_path.includes('/changeEmail')) {
                              resendCodeOnOptinAfterReg(configKey, options);
                            } else {
                              $__default["default"].ajax({
                                ...
                                success: function success(t) {
                                  if (options.onResendCode) {
                                    options.onResendCode(t);
                                  }
                                  if (t.message) {
                                    showPopup(t.message);
                          ...
                          case 17:
                            ...
                            break;
                          case 21:
                            if (!(event.indexOf("social-login-") > -1)) {
                              _context.next = 29;
                              break;
                            }
                            $__default["default"](".ube-validation-message-show-for-social-login").hide();
                            toggleLoader(true);
                            key = event.substring(13);
                            gaPush$1(categoryPrefix + "auth_social_start", "click - icon: ".concat(key));
                            $__default["default"].ajax({
                              url: api_url + "/auth/" + name + "/" + key + "/url",
                              headers: {
                                "Authorization": "Bearer " + sessionKey
                            ...
                            break;
                          case 29:
                            if (!(event.indexOf("social-popup-login-") > -1)) {
                              _context.next = 35;
                              break;
                            }
                            key = event.substring(19);
                            gaPush$1("Social-Login", "Shop-Popup", key);
                            ...
                            break;
                          case 35:
                            if (!(event.indexOf("social-set-") > -1)) {
                              _context.next = 42;
                              break;
                            }
                            key = event.substring(11);
                            gaPush$1("Social-Attach", "Shop-Popup", key);
                            finalUrl = $__default["default"](this).attr("data-ube-url");
                            ...
                            break;
                          case 42:
                            if (!(event.indexOf("optin-cancel-") > -1)) {
                              _context.next = 54;
                              break;
                            }
                            key = event.substring(13);
                            if (!(["email", "phone", "cross", "personal"].indexOf(key) === -1)) {
                              _context.next = 46;
                              break;
                            }
                            return _context.abrupt("return", console.error("Optin validation only available for phone and email fields, not for " + key));
                          case 46:
                            setFieldValue(key + "CodeSent", false);
                            setFieldValue(key + "ValidatedValue", "");
                            setFieldValue(key + "CodeSentAt", null);
                            ...
                            break;
                          case 54:
                            if (!(event.indexOf("optin-validate-") > -1)) {
                              _context.next = 62;
                              break;
                            }
                            key = event.substring(15);
                            if (!(["email", "phone", "cross", "personal"].indexOf(key) === -1)) {
                              _context.next = 58;
                              break;
                            }
                          ...
                          case 58:
                            ...
                            break;
                          case 62:
                            if (options.onSubmissionSuccess) {
                  ...
                  if (isWizard) {
                    ...
                    if (isMBEPForm) {
                      gaPush$1('dl_av_idx_registration_start', 'form - idx_reg');
                    }
                    var changeStep = function changeStep(delta, b, e) {
                      ...
                      if (isMBEPForm && newStepIndex === 1 && currentStepIndex === 0) {
                        gaPush$1('dl_av_idx_registration_process', 'send - form - idx_reg');
                        gaPush$1('dl_av_idx_registration_success', 'form - idx_reg');
                        gaPush$1('dl_auth_registration_start', 'form - registration');
                      }
                ...
                isTemplatePreloaded = options.isTemplatePreloaded && !isNoPreload(name);
      ...
      } else if (options.formDefinitionJson) initializeFormByConfig(options.formDefinitionJson)["catch"](handleAjaxFail);else {
        $__default["default"].getJSON(updatedUrl).done(function (json) {
          return initializeFormByConfig(json)["catch"](handleAjaxFail);
        }).fail(handleAjaxFail);
      }
      return this;
    };
