var library = (function (exports, $$1) {
    ...
    */
    ...
    function getToken(callback, repeat) {
      ...
      if (!repeat) repeat = MAX_REPEAT_COUNT;
      $.getJSON("https://ube-test.pmsm.org.ru/api/face/token").done(function (data) {
        toggleLoader(false);
    ...
    (function () {
            ...
            success: function success(data) {
              if (priorityFunction) {
                ...
                if (data.isAdult) {
                  ...
                  setSessionData(data, null, faceState === null || faceState === void 0 || (_faceState$options = faceState.options) === null || _faceState$options === void 0 ? void 0 : _faceState$options.onSubmissionSuccess);
                } else {
                  if (tryCount < 2) {
                    tryCount++;
                    fcm.reload();
      ...
      $__default["default"].fn.ubeFace = function (name, options) {
        ...
        function initializeFaceCapture(_token, _tokenUrl) {
          ...
          var captureButton = container.find(".ube-camera-capture");
          var fallbackTarget = container.find(".ube-camera-fallback");
          if (renderTarget.data("init") || !_token || !_tokenUrl) return false;
          renderTarget.data("init", true);
          ...
          function renderCameraCapture() {
            ...
            var onFaceError = function onFaceError(err) {
              ...
              var countlabel = tryCount === 1 ? 'first' : 'second';
              console.log('Face capture error:', errLabel);
              gaPush("dl_qr_av_fr_process", "start - form - fr_upload - reason: ".concat(errLabel, " - ").concat(countlabel));
              renderFileUpload();
            };
            var onReadyForCaptureFace = function onReadyForCaptureFace() {
              var countlabel = tryCount === 1 ? 'first' : 'second';
              gaPush("dl_qr_av_fr_process", "start - form - fr_auto - ".concat(countlabel));
