      ...
      $__default["default"].fn.ubeFace = function (name, options) {
        ...
        function initializeFaceCapture(_token, _tokenUrl) {
          ...
          function sendBlobToServer(blob, callback, type, imageData) {
            ...
            gaPush("dl_qr_av_fr_process", "send - form - fr_".concat(type, " - ").concat(label));
              ...
              success: function success(data) {
                ...
                if (data) {
                  ...
                  if (data.status_code === "SUCCESS" && !window.bad) {
                    var _label = tryCount === 1 ? 'first' : 'second';
                    gaPush("dl_qr_av_fr_success", "form - fr_".concat(type, " - ").concat(_label));
                    ...
                    onError("limit", data);
                    var _label2;
                    if (errorMessage === "Technical Error") _label2 = 'technical';else if (tryCount === 1) _label2 = 'first';else _label2 = 'second';
                    ...
                    if (data.errors_detail) {
                      ...
                      var _label4;
                      if (_errorMessage === "Technical Error") _label4 = 'technical';else if (tryCount === 1) _label4 = 'first';else _label4 = 'second';
                      gaPush("dl_qr_av_fr_fail", "form - fr_".concat(type, " - ").concat(_label4, " - reason: ").concat(_errorMessage));
                      logACSResultToELK(eventType, false, false, _errorMessage, imageData, b64Img, name);
                    } else gaPush("dl_qr_av_fr_fail", "form - fr_".concat(type, " - ").concat(_label2, " - reason: ").concat(errorMessage));
                  } else if (data.error_message) {
                    console.log(data.error_message);
                    if (!data.error_message.includes('Yoti does not recognize')) {
                      var _label5;
                      if (errorMessage === "Technical Error") _label5 = 'technical';else if (tryCount === 1) _label5 = 'first';else _label5 = 'second';
                      gaPush("dl_qr_av_fr_fail", "form - fr_".concat(type, " - ").concat(_label5, " - reason: ").concat(errorMessage));
                    }
                    ...
                    if (data.errors_detail) {
                      ...
                      var _label6;
                      if (_errorMessage2 === "Technical Error") _label6 = 'technical';else if (tryCount === 1) _label6 = 'first';else _label6 = 'second';
                      gaPush("dl_qr_av_fr_fail", "form - fr_".concat(type, " - ").concat(_label6, " - reason: ").concat(_errorMessage2));
                    }
                    if (tryCount < 2) {
                      tryCount++;
                      fcm.reload();
