                  ...
                  case 0:
                    ...
                    /*try {
                        var result = await $.ajax({
                            url: `${host}/api/log/verifyFace/${configName}`,
                    ...
                    $__default["default"].ajax({
                      ...
                      error: function error(err) {
                        toggleLoader(false);
                        logACSResultToELK(eventType, false, true, 'Network Error', imageData, imageBase64, configName, void 0 );
                      },
                      success: function success(data) {
                        toggleLoader(false);
                        if (data) {
                          ...
                          if (data.status_code === "SUCCESS" && !window.bad) {
                            ...
                            logACSResultToELK(eventType, true, false, null, imageData, imageBase64, configName, token);
                            if (callback) callback();
                            onSuccess(token);
                            ...
                            onError("limit", data);
                            var _label;
                            if (errorMessage === "Technical Error") _label = 'technical';else _label = 'first';
                            ...
                            logACSResultToELK(eventType, false, false, detailMessage, imageData, imageBase64, configName);
                            if (data.errors_detail) {
                              var _errorMessage = data.errors_detail[0] && data.errors_detail[0].code || "Technical Error";
                              var _label3;
                              if (_errorMessage === "Technical Error") _label3 = 'technical';else _label3 = 'first';
                              gaPush("dl_qr_av_fr_fail", "form - fr_".concat(type, " - ").concat(_label3, " - reason: ").concat(_errorMessage));
                              logACSResultToELK(eventType, false, false, _errorMessage, imageData, imageBase64, configName);
                            } else gaPush("dl_qr_av_fr_fail", "form - fr_".concat(type, " - ").concat(_label, " - reason: ").concat(errorMessage));
                          } else if (data.error_message) {
                            console.log(data.error_message);
                            if (!data.error_message.includes('Yoti does not recognize')) {
                              var _label4;
                              if (errorMessage === "Technical Error") _label4 = 'technical';else _label4 = 'first';
                            ...
                            logACSResultToELK(eventType, false, false, _detailMessage, imageData, imageBase64, configName);
                            if (data.errors_detail) {
                              ...
                              var _label5;
                              if (_errorMessage2 === "Technical Error") _label5 = 'technical';else _label5 = 'first';
                    ...
                    img.onload = function () {
                      faceapi.detectSingleFace(img, new faceapi.TinyFaceDetectorOptions({
                        ...
                        if (!res) cb(resolve);else {
                          console.log(5555, img.width, img.height);
                          var imageData = {
                            width: img.width,
                            height: img.height,
                          ...
                          console.log(6666, aggregatedImageData);
                          resolve({
                            image: img.src,
    ...
    var Face = /*#__PURE__*/function (_Block) {
      ...
      return _createClass(Face, [{
        ...
        value: function run(key, state, cb) {
          ...
          return target.ubeFaceLocal(faceName, _objectSpread2({
            isTemplatePreloaded: false,
            onFormLoad: function onFormLoad() {
              options.onFormLoad && options.onFormLoad(key);
            },
