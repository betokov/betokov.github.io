    ...
    */
    ...
    var getQueryParameter = function getQueryParameter(query, sParam) {
      ...
      for (i = 0; i < sURLVariables.length; i++) {
        sParameterName = sURLVariables[i].split(/=([\s\S]*)/);
    ...
    var gaSkipEmailOptin = function gaSkipEmailOptin(category, action) {
      if (ubeSession('shouldSkipEmail') === 'true') {
        gaPush(category, "".concat(action, " - with_email"));
      } else if (ubeSession('shouldSkipEmail') === 'false') {
        if (ubeSession('skipedEmailOptin') === 'true') {
          gaPush(category, "".concat(action, " - wo_email"));
        } else {
          gaPush(category, "".concat(action, " - wo_email_leave_email"));
        }
      }
    };
    ...
    function send(data) {
      ...
      return fetch("http://localhost:8080/api/impression/token", options).then(function (response) {
        ...
        return _regeneratorRuntime().wrap(function _callee$(_context) {
          while (1) switch (_context.prev = _context.next) {
            case 0:
              _context.prev = 0;
              _context.next = 3;
              return fetch("http://localhost:8080/api/config/preload/".concat(key));
            case 3:
            ...
            case 8:
              data = _context.sent;
              if (data.status) gaPush('dl_qr_preload', 'get - configuration_preload');
              _context.next = 14;
              break;
            case 12:
              _context.prev = 12;
      ...
      $.getJSON("".concat($.ube.host, "/api/face/token")).done(function (data) {
        ...
        if (!data.access_token || !data.next_validation_endpoint) {
          ...
          if (repeat > 0) setTimeout(function () {
            ...
            gaPush("AV", "".concat(categoryPrefix, "av_fr_fail"), "form - fr - reason: Get-Token-Error-UBE");
            showPopup("Ошибка в ответе сервиса ACS API");
          }
        } else {
          gaPush("".concat(categoryPrefix, "av_fr_start"), "form - fr");
          callback(data.access_token, data.next_validation_endpoint);
        }
      }).fail(function (jqxhr, textStatus, error) {
        ...
        if (repeat > 0) setTimeout(function () {
          ...
          gaPush("AV", "".concat(categoryPrefix, "av_fr_fail"), "form - fr - reason: Get-Token-Error-ACS");
          showPopup("Ошибка в ответе сервиса ACS API");
        }
      });
    }
    ...
    function initFace(container, initializeFaceCapture, state) {
      container.find(".ube-visibility-show-for-method, .ube-visibility-show-for-megafonMethod, .ube-visibility-show-for-documentMethod").hide();
      container.find(".ube-visibility-show-for-faceMethod").show();
      gaPush("qr_age", "fr", state.promoCampaignName);
        ...
        var xhrRequest = $.ajax({
          ...
          error: function error(err) {
            if (priorityFunction) {
              ...
              gaPush("".concat(categoryPrefix, "av_fr_fail"), "form - fr_".concat(type, " - ").concat(getAttemptLabel(faceState.tryCount), " - reason: technical"));
          ...
          success: function success(data) {
            if (priorityFunction) {
              ...
              if (data.isAdult) {
                ...
                gaPush("".concat(categoryPrefix, "av_fr_success"), "form - fr_".concat(type, " - ").concat(getAttemptLabel(faceState.tryCount)));
                data.jwt && ubeCookie(cookies.UBE_JWT_FACE_TOKEN, data.jwt);
                faceState.onSuccess(data);
              } else {
                gaPush("".concat(categoryPrefix, "av_fr_fail"), "form - fr_".concat(type, " - ").concat(getAttemptLabel(faceState.tryCount)));
                if (faceState.tryCount < 2) {
                  faceState.tryCount++;
                  faceState.fcm.reload();
            ...
            if (data) {
              ...
              if (status_code === "SUCCESS" && !window.bad) {
                ...
                if (priorityFunction) {
                  ...
                  gaPush("".concat(categoryPrefix, "av_fr_success"), "form - fr_".concat(type, " - ").concat(label));
                  if (type === 'auto') faceState.fcm.unmount();
                ...
                if (priorityFunction) {
                  if (type === 'auto') faceState.fcm.unmount();
                  faceState.onError("limit", data);
                  gaPush("".concat(categoryPrefix, "av_fr_fail"), "form - fr_".concat(type, " - ").concat(getAttemptLabel(faceState.tryCount, error_message), " - reason: ").concat(error_message));
                }
                ...
                if (priorityFunction) {
                  ...
                  if (errors_detail) {
                    ...
                    gaPush("".concat(categoryPrefix, "av_fr_fail"), "form - fr_".concat(type, " - ").concat(getAttemptLabel(faceState.tryCount, _error_message), " - reason: ").concat(_error_message));
                  }
                  if (faceState.tryCount < 2) {
      ...
      $__default["default"].fn.ubeFace = function (name, options) {
        ...
        function initializeFaceCapture(_token, _tokenUrl) {
          ...
          function sendBlobToServer() {
            ...
            gaPush("".concat(categoryPrefix, "av_fr_process"), "send - form - fr_".concat(faceState.type, " - ").concat(label));
          ...
          function renderCameraCapture() {
            ...
            var onFaceError = function onFaceError(err) {
              ...
              gaPush("".concat(categoryPrefix, "av_fr_process"), "start - form - fr_upload - reason: ".concat(errLabel, " - ").concat(label));
            ...
            var onReadyForCaptureFace = function onReadyForCaptureFace() {
              !$__default["default"].ube.host.includes('test') && toggleLoader(false);
              var label = faceState.tryCount === 1 ? 'first' : 'second';
              gaPush("".concat(categoryPrefix, "av_fr_process"), "start - form - fr_auto - " + label);
    ...
    function initPreferredMethod(_ref) {
      ...
      if (preferredMethodAfterPhone) {
        $('.ube-select-preferredMethod').on('click', function () {
          ...
          gaPush("".concat(category, "phone_process"), 'view - screen_fork');
      ...
      methodBtn.on('click', function (e) {
        ...
        if (method === 'telegram') {
          ...
          gaPush("".concat(category, "phone_process"), "click - phone_telegram_push - screen_fork");
          gaPush("".concat(category, "phone_start"), "form - phone_telegram_push");
        } else if (method === 'sms') {
          gaPush("".concat(category, "phone_process"), "click - phone_sms_code - screen_fork");
          gaPush("".concat(category, "phone_start"), "form - phone_sms_code");
        } else if (method === 'flashingCall') {
          gaPush("".concat(category, "phone_process"), "click - phone_call_code - screen_fork");
          gaPush("".concat(category, "phone_start"), "form - phone_call_code");
        }
            ...
            case 0:
              _messageHandle = function _messageHandle3() {
                _messageHandle = _asyncToGenerator(/*#__PURE__*/_regeneratorRuntime().mark(function _callee(event) {
                  var _getParameters, _getParameters2, utmCampaign, utmSource, utmContent, utmMedium, utmTerm, method, data, response, result;
                  return _regeneratorRuntime().wrap(function _callee$(_context) {
                    while (1) switch (_context.prev = _context.next) {
                      case 0:
                        ...
                        return _context.abrupt("return");
                      case 2:
                        _getParameters = getParameters('utm_campaign', 'utm_source', 'utm_content', 'utm_medium', 'utm_term', 'method'), _getParameters2 = _slicedToArray(_getParameters, 6), utmCampaign = _getParameters2[0], utmSource = _getParameters2[1], utmContent = _getParameters2[2], utmMedium = _getParameters2[3], utmTerm = _getParameters2[4], method = _getParameters2[5];
                        data = {
    ...
    var Content = /*#__PURE__*/function (_Block) {
      ...
      return _createClass(Content, [{
        ...
        value: function run(key, state, cb) {
          var categoryPrefix = getCategoryPrefix();
          gaPush("".concat(categoryPrefix, "content_start"), '');
          this.options.skuContent(function () {
            gaPush("".concat(categoryPrefix, "content_end"), '');
            cb({
                    ...
                    onResendCode: function onResendCode() {
                      if (resendCounter >= MAX_RESEND_COUNT) {
                        $('.skip-optin').show().on('click', function () {
                          ...
                          gaPush("".concat(categoryPrefix, "2optin_process"), 'click - skip - form - 2optin');
      ...
      return _createClass(Face, [{
        ...
        value: function run(key, state, cb) {
          ...
          if (state.shouldSkipFace) {
            gaPush("".concat(categoryPrefix, "av_fr_skip"), 'click - skip - form - fr');
            return cb({
              avStatus: 1
            }, faceSuccess);
          }
          gaPush("".concat(categoryPrefix, "av_fr_process"), 'view - screen_fr');
      ...
      var _loop = function _loop(login) {
        if (logins[login]) {
          $(".ube-auth-".concat(login)).on('click', function () {
            gaPush("".concat(categoryPrefix, "auth_social_start"), "click - icon: ".concat(socialNamesMapping[login]));
            toggleLoader(true);
    ...
    function onSocialAuthSuccess(_ref2) {
      ...
      switch (result.event) {
        case "authorization_not_found":
          if (!(result !== null && result !== void 0 && (_result$user = result.user) !== null && _result$user !== void 0 && _result$user.phone)) {
            ...
            gaPush("".concat(categoryPrefix, "auth_social_fail"), "view - pop_up_fail_social - ".concat(socialNamesMapping[socialName]));
            toggleLoader(false);
      ...
      gaPush("".concat(categoryPrefix, "auth_social_success"), "form - social - ".concat(socialNamesMapping[socialName]));
    }
    ...
    */
    ...
    (function () {
      $__default["default"].fn.ubeScanpackLogin = function (name, callback, options) {
        ...
        var authorizationEventSource = null;
        var isFRCookieExists = ubeCookie$1(cookies.UBE_JWT_FACE_TOKEN) || false;
        ...
        function loadPhoneSend() {
          ...
          phoneSendContainer.show().ube(phoneSendName, {
            ...
            onSubmissionSuccess: function onSubmissionSuccess(submissionId, userId, result) {
              ...
              switch (result.event) {
                case 'success':
                  loadPhoneCheck(result.data.sessionKey, result.data.phone, (_result$data = result.data) === null || _result$data === void 0 ? void 0 : _result$data.phoneToCall);
                  break;
              }
            }
          });
        ...
        function stopAuthorizationCheck() {
          if (authorizationEventSource) {
            authorizationEventSource.close();
            authorizationEventSource = null;
          }
        }
        function startAuthorizationCheck(sessionKey, phone) {
          authorizationEventSource = new EventSource("".concat($__default["default"].ube.host, "/api/authorization/event/").concat(name), {
          ...
          authorizationEventSource.onerror = function () {
            var _authorizationEventSo;
            var isReconnecting = ((_authorizationEventSo = authorizationEventSource) === null || _authorizationEventSo === void 0 ? void 0 : _authorizationEventSo.readyState) === EventSource.CONNECTING;
            if (isReconnecting) return;
            stopAuthorizationCheck();
            backToPhoneSend();
            showPopup("Что-то пошло не так, попробуйте ещё раз");
          };
          authorizationEventSource.addEventListener('message', function (result) {
            var authResponse;
            try {
              authResponse = {
            ...
            var _authResponse$data = authResponse.data,
              event = _authResponse$data.event,
              sessionKey = _authResponse$data.sessionKey;
            switch (event) {
              case 'authorization_not_found':
                showRegistrationPage(sessionKey, phone);
                break;
              case 'authorization_success':
                setCookies(sessionKey, 'login');
                getInfo(sessionKey, name).done(function (response) {
                  var avStatus = +response.av;
                  if (isIDXEnabled) {
                    if (avStatus === 1) {
                      callback('loginSuccess', authResponse, 'login', 1);
                    } else {
                      showAVPage(response.sessionKey, 'login');
                    }
                  } else {
                    callback('loginSuccess', authResponse, 'login', avStatus);
                  }
                });
                break;
              case 'no_pending_record':
              case 'auth_call_expired_event':
              case 'no_auth_call_key_event':
                {
                  var _authResponse$data2;
                  backToPhoneSend();
                  showPopup((_authResponse$data2 = authResponse.data) === null || _authResponse$data2 === void 0 ? void 0 : _authResponse$data2.message);
                  break;
                }
              case 'auth_call_pending_for_call_event':
                {
                  return;
                }
            }
        ...
        function loadPhoneCheck(sessionKey, phone, phoneToCall) {
          ...
          phoneCheckContainer.show().ube(phoneCheckName, {
            ...
            onFormLoad: function onFormLoad() {
              handleTextVisibility();
              if (preferredMethod) {
                if (preferredMethod === 'authCall' && phoneToCall) {
                  var authCallContainer = $__default["default"]('.ube-phone-check-auth-call-container');
                  startAuthorizationCheck(sessionKey, phone);
                  $__default["default"]('.ube-phone-check-form').hide();
                  $__default["default"](authCallContainer).find('.ube-phone-check-auth-call-button').attr('href', "tel:+".concat(phoneToCall));
                  $__default["default"](authCallContainer).find('.ube-phone-check-auth-call-phone').text("+".concat(phoneToCall));
                  $__default["default"](authCallContainer).show();
                }
    ...
    function validateYandexCaptcha(container, callback) {
      ...
      if (captchaId) {
        ...
        gaPush("".concat(categoryPrefix, "captcha_start"), label);
        if (widgetId == undefined || widgetId == null) {
          ...
          window.smartCaptcha.subscribe(widgetId, 'challenge-visible', function () {
            return gaPush("".concat(categoryPrefix, "captcha_process"), label.replace('form', 'view'));
          });
                ...
                setFieldInvalid = function _setFieldInvalid(key, value, description) {
                  ...
                  if (key && gaFieldSet[key] !== false) {
                    ...
                    if (/-phone-send$/.test(name)) {
                      ...
                      gaPush(_category, action);
                    } else if (/-phone-check$/.test(name)) {
                      var _category2 = isQRForm ? 'dl_qr_phone_fail' : 'dl_auth_phone_fail';
                      ...
                      gaPush(_category2, _action);
                    } else if (/-reg$/.test(name)) {
                      var _category3 = isQRForm ? 'dl_qr_registration_fail' : 'dl_auth_registration_fail';
                      if (isMBEPForm && $__default["default"]('.reg-form_step_0').is(':visible')) {
                        ...
                        gaPush(_category3, "form - registration - ".concat(key, " - reason: ").concat(description));
                      }
              ...
              case 46:
                ...
                handle_form = function handle_form() {
                  ...
                  if (/-reg$/.test(path)) {
                    ...
                    var _category4 = isQRForm ? 'dl_qr_registration_start' : 'dl_auth_registration_start';
                    if (!isMBEPForm) {
                      if (ubeSession('shouldSkipEmail') === 'true') {
                        gaPush(_category4, 'form - registration - with_email');
                      } else if (ubeSession('shouldSkipEmail') === 'false') {
                        gaPush(_category4, 'form - registration - wo_email');
                      }
                      gaPush(_category4, "form - registration");
                    }
                    if (isMBEPForm) {
                      gaPushObject({
                        event: 'dl-pageview',
                    ...
                    var _category5 = categoryPrefix + '2optin_start';
                    var action = 'form - 2optin';
                    gaSkipEmailOptin(_category5, action);
                    ...
                    if (!isMBEPForm && !isQRForm) {
                      ...
                      $__default["default"]('input[name=submittedPhoneCode]').one('input', function () {
                        ...
                        gaPush('dl_auth_phone_process', _action2);
                      });
                    }
                    ...
                    if (isMBEPForm) {
                      ...
                      gaPush('dl_auth_phone_process', 'view - screen_phone_number');
                    }
                    if (!isMBEPForm && !preferredMethod) {
                    ...
                    $__default["default"].ajax({
                      ...
                      error: function error(xhr, resp, text) {
                        ...
                        if (/-reg$/.test(path)) {
                          ...
                          var _category6 = isQRForm ? 'dl_qr_registration_fail' : 'dl_auth_registration_fail';
                          gaPush(_category6, "form - registration - reason: technical");
                          if (isMBEPForm) {
                          ...
                          var _category7 = isQRForm ? 'dl_qr_phone_fail' : 'dl_auth_phone_fail';
                          ...
                          gaPush(_category7, _action3);
                        } else if (/-phone-send$/.test(path)) {
                          action_path = "/esb/" + path + "/submission";
                          var _category8 = isQRForm ? 'dl_qr_phone_fail' : 'dl_auth_phone_fail';
                      ...
                      function submissionSuccess(result) {
                        ...
                        if (name.includes('phone-send')) {
                          var _category9 = isQRForm ? 'dl_qr_phone_process' : 'dl_auth_phone_process';
                          ...
                          var _category10 = isQRForm ? 'dl_qr_phone_success' : 'dl_auth_phone_success';
                          ...
                          var _category11 = isQRForm ? 'dl_qr_registration_success' : 'dl_auth_registration_success';
                          var _action7 = 'form - registration';
                          gaSkipEmailOptin(_category11, _action7);
                          gaPush(_category11, _action7);
                          ...
                          var _category12 = categoryPrefix + '2optin_success';
                          var _action8 = 'form - 2optin';
                          gaSkipEmailOptin(_category12, _action8);
                          gaPush(_category12, _action8);
                        } else if (name.includes('-av')) {
                          if (!hasField('faceMethod') || data.data.method === 'document') {
                            gaPush(categoryPrefix + 'av_idx_success', 'form - idx - document: ' + payload.data.documentType);
                          }
                        ...
                        if (ga_name === 'LoginRemind') gaPush(categoryPrefix + 'auth_resetPassword_success', 'form - resetPassword');else if (ga_name === 'Login') {
                          ...
                          gaPush(categoryPrefix + 'auth_password_success', "form - password - ".concat(_action9));
                        }
                  ...
                  function onFormSubmit(part) {
                    ...
                    validateAllFields(function () {
                      ...
                      validationState = {};
                      if (/-reg$/.test(path)) {
                        var _category13 = isQRForm ? 'dl_qr_registration_process' : 'dl_auth_registration_process';
                        gaPush(_category13, 'send - form - registration');
                      } else if (/optin$/.test(path)) {
                        actionPathOptin({
                          path: path,
                          configKey: configKey,
                        ...
                        var _category14 = categoryPrefix + '2optin_process';
                        var _action10 = 'send - form - 2optin';
                        gaSkipEmailOptin(_category14, _action10);
                        gaPush(_category14, _action10);
                        ga_name = "2OPTIN";
                      } else if (/av$/.test(path)) {
                        action_path = "/esb/" + path + "/submission";
                        ga_name = "AV";
                        ...
                        var _category15 = isQRForm ? 'dl_qr_phone_process' : 'dl_auth_phone_process';
                        ...
                        gaPush(_category15, _action11);
                      } else if (/-phone-send$/.test(path)) {
                        var _category16 = isQRForm ? 'dl_qr_phone_process' : 'dl_auth_phone_process';
                        ...
                        if (isMBEPForm && !isP1Domain()) {
                          ...
                          isQRDomain() ? 'dl_qr_' : 'dl_auth_';
                          _action12 = _phoneTypeActions7[phoneConfirmationType];
                        }
                        gaPush(_category16, _action12);
                      } else if (/-idx$/.test(path)) {
                        action_path = "/esb/" + path + "/submission";
                        ...
                        while (1) switch (_context.prev = _context.next) {
                          ...
                          case 3:
                            ...
                            if (event.includes('close') && /-phone-check$/.test(path)) {
                              ...
                              gaPush("".concat(category, "phone_process"), action);
