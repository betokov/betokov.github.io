    ...
    */
    ...
    $__default["default"].fn.ube = function (url, options) {
      ...
      function initializeFormByConfig(json) {
        ...
        var categoryPrefix = isQRForm ? 'dl_qr_' : 'dl_';
        var phoneSendMethod = sessionStorage.getItem('phoneSendMethod');
        var remainingAttemptsCount, preferredMethodContainer;
        ...
        function setFieldInvalid(key, value, description) {
          ...
          if (key && gaFieldSet[key] !== false) {
            ...
            if (/-phone-send$/.test(name)) {
              ...
              var action;
              if (preferredMethodContainer) {
                action = phoneSendMethod === 'telegram' ? "number - form - phone_telegram_push - reason: ".concat(description) : "number - form - phone_sms_code - reason: ".concat(description);
              } else {
        ...
        var handle_form = function handle_form() {
          ...
          var action_path;
          var ga_name;
          preferredMethodContainer = $__default["default"]('.ube-phone-send-preferredMethod').length;
            ...
            $__default["default"].ajax({
              ...
              error: function error(xhr, resp, text) {
                ...
                if (/-reg$/.test(path)) {
                  ...
                  if (isP1Form) gaPush('dl_auth_phone', 'fail - phone', 'technical');else {
                    if (preferredMethodContainer) {
                      action = phoneSendMethod === 'telegram' ? 'number - form - phone_telegram_push - reason: technical' : 'number - form - phone_sms_code - reason: technical';
                    } else {
              ...
              function submissionSuccess(result) {
                ...
                if (name.includes('phone-send')) {
                  ...
                  var action;
                  if (preferredMethodContainer) {
                    action = phoneSendMethod === 'telegram' ? 'send - telegram_push_to_las - form - phone_telegram_push' : 'send - sms_code_to_las - form - phone_sms_code';
                  } else {
                    action = 'send - code';
                  }
                  ...
                  var _action3;
                  if (preferredMethodContainer) {
                    _action3 = phoneSendMethod === 'telegram' ? "form - phone_telegram_push - (".concat(result.event === 'loginSuccess' ? 'in_base' : 'not_in_base', ")") : "form - phone - (".concat(result.event === 'loginSuccess' ? 'in_base' : 'not_in_base', ")");
                  } else {
          ...
          function onFormSubmit(part) {
            ...
            validateAllFields(function () {
              ...
              if (/-reg$/.test(path)) {
                ...
                var _action5;
                if (preferredMethodContainer) {
                  _action5 = phoneSendMethod === 'telegram' ? 'send - number - form - phone_telegram_push' : 'send - number - form - phone_sms_code';
                } else {
          ...
          container.off("click", ".ube-trigger-event").on("click", ".ube-trigger-event", function (e) {
            ...
            if (event.includes('close') && isQRForm) {
              if (preferredMethodContainer) {
                action = phoneSendMethod === 'telegram' ? 'click - button - change_phone - form - phone_telegram_push' : 'click - button - change_phone - form - phone_sms_code';
              } else {
