    ...
    */
    ...
    if (window.opener) {
      ...
      if (payload.uuid && payload.token) {
        window.opener.postMessage(JSON.stringify(payload), 'http://localhost:8081');
      }
      window.close();
    }
            ...
            case 0:
              _messageHandle = function _messageHandle3() {
                _messageHandle = _asyncToGenerator(/*#__PURE__*/_regeneratorRuntime().mark(function _callee(event) {
                  var _JSON$parse, token, uuid, _getParameters, _getParameters2, utmCampaign, utmSource, utmContent, utmMedium, utmTerm, response, result;
                  return _regeneratorRuntime().wrap(function _callee$(_context) {
                    while (1) switch (_context.prev = _context.next) {
                      case 0:
                      ...
                      case 2:
                        _JSON$parse = JSON.parse(event === null || event === void 0 ? void 0 : event.data), token = _JSON$parse.token, uuid = _JSON$parse.uuid;
                        console.log(111, token, uuid);
      ...
      return _createClass(PhoneSend, [{
        ...
        value: function run(key, state, cb) {
          ...
          function onSocialAuthSuccess(result, socialName) {
            ...
            switch (result.event) {
              case "authorization_not_found":
                if (!(result !== null && result !== void 0 && (_result$user = result.user) !== null && _result$user !== void 0 && _result$user.phone)) {
                  ...
                  // gaPush('dl_qr_auth_social_fail', `view - pop_up_fail_social - ${socialNamesMapping[socialName]}`);
            ...
            // gaPush('dl_qr_auth_social_success', `form - social - ${socialNamesMapping[socialName]}`);
          }
          target.show().ube(phoneSendName, {
            ...
            onFormLoad: function onFormLoad(_ref) {
              ...
              if (hasPreferredMethodContainer) gaPush('dl_qr_phone_process', 'view - screen_fork');
              ...
              var _loop = function _loop(login) {
                if (logins[login]) {
                  $(".ube-auth-".concat(login)).on('click', function () {
                    // gaPush('dl_qr_auth_social_start', `click - icon: ${socialNamesMapping[login]}`);
      ...
      $__default["default"].fn.ubeScanpackLogin = function (name, callback, options) {
        ...
        function onSocialAuthSuccess(result, socialName) {
          ...
          var _result$user = result.user,
            phone = _result$user.phone,
            sessionKey = _result$user.sessionKey;
          switch (result.event) {
            case "authorization_not_found":
              if (!phone) {
