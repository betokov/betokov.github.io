var library = (function (exports, $$1) {
    ...
    var ubeSession$1 = function ubeSession(key, value) {
    ...
    */
    ...
    var gaSkipEmailOptin = function gaSkipEmailOptin(category, action) {
      if (ubeSession$1('shouldSkipEmail') === 'true') {
        gaPush(category, "".concat(action, " - with_email"));
      } else if (ubeSession$1('shouldSkipEmail') === 'false') {
        if (ubeSession$1('skipedEmailOptin') === 'true') {
      ...
      return _createClass(AV, [{
        ...
        value: function run(key, state, cb) {
          ...
          target.show().ubeAV("".concat($.ube.host, "/main/").concat(name), sessionKey, {
            data: {
              entity: name,
              skipEmailOptin: ubeSession('skipedEmailOptin')
            },
            isTemplatePreloaded: false,
    ...
    var EmailOptin = /*#__PURE__*/function (_Block) {
      ...
      return _createClass(EmailOptin, [{
            ...
            onSubmissionSuccess: function onSubmissionSuccess(submissionId, userId, result) {
              ...
              switch (result.event) {
                case "alreadyVerified":
                  ...
                  ubeSession$1('skipedEmailOptin', null);
        ...
        value: function close() {
          ...
          ubeSession$1('shouldSkipEmail', null);
        }
      }]);
    }(Block);
        ...
        value: function run(key, state, cb) {
          ...
          target.show().ube(phoneSendName, {
            ...
            onFormLoad: function onFormLoad(_ref) {
              ...
              var hasPreferredMethodContainer = !!$('.ube-phone-send-preferredMethod').length;
              var methodBtn = $('.method-btn');
              var category = isQRDomain() ? 'dl_qr_' : 'dl_auth_';
              if (hasPreferredMethodContainer || methodBtn.length) gaPush("".concat(category, "phone_process"), 'view - screen_fork');
        ...
        value: function run(key, state, cb) {
          ...
          target.show().ube(this.name, {
            ...
            onFormLoad: function onFormLoad() {
              ...
              if (ubeSession$1('shouldSkipEmail') === 'false') {
                ...
                $('.skipEmailOptin').on('click', function () {
                  ...
                  if (skipEmailOptinValue === true) {
                    ...
                    ubeSession$1('skipedEmailOptin', true);
                  } else {
                    $('.ube-registration-switch').hide();
                    $('.ube-registration-block').show();
                    ubeSession$1('skipedEmailOptin', false);
                  }
                });
              } else {
                $('.ube-registration-switch').hide();
        ...
        function onSocialAuthSuccess(result, socialName) {
          ...
          switch (result.event) {
            ...
            case 'authorization_success':
              ...
              getAvStatus(sessionKey, name).done(function (response) {
                var av = response.av,
                  trialAccess = response.trialAccess;
                var avStatus = +av;
                if (isIDXEnabled || isDocuments || isTrial) {
    ...
    function validateYandexCaptcha(container, callback) {
      ...
      if (captchaId) {
        ...
        var categoryPrefix = getCategoryPrefix();
        var preferredMethod = ubeSession$1('preferredMethod');
        var phoneConfirmationType = ubeSession$1('phoneConfirmationType');
        ...
        if (widgetId == undefined || widgetId == null) {
          var sitekey = ubeSession$1('sitekey');
    ...
    var setSocialIdCookie = function setSocialIdCookie(socialId, socialKey, result) {
      autoAddSessionFields.forEach(function (key) {
        if (result[key]) ubeSession$1(key, result[key]);
      });
      ubeSession$1("socialId", socialId);
      ubeSession$1("socialKey", socialKey);
    };
    var clearSessionFields = function clearSessionFields() {
      autoAddSessionFields.forEach(function (key) {
        ubeSession$1(key, null);
      });
    };
    ...
    $__default["default"].fn.ube = function (url, options) {
      ...
      function _initializeFormByConfig() {
        _initializeFormByConfig = _asyncToGenerator(/*#__PURE__*/_regeneratorRuntime().mark(function _callee(json) {
            ...
            while (1) switch (_context.prev = _context.next) {
              case 0:
                ...
                getFormData = function _getFormData(part) {
                  ...
                  if (!isLeadForm) {
                    formData.isFaceSkipped = options.isFaceSkipped || JSON.parse(ubeSession$1('isFaceSkipped'));
                  ...
                  fieldNamesByPart(part).forEach(function (key) {
                    ...
                    if (!formData[key] && ubeSession$1(key)) formData[key] = ubeSession$1(key);
                  });
                  formData.service = {
                ...
                configFromSession = JSON.parse(ubeSession$1("".concat(configKey, "_config")));
                if (!safeOptionData) {
                  _context.next = 37;
                  break;
                }
              ...
              case 47:
                ...
                handle_form = function handle_form() {
                  var _options$config, _json$properties;
                  var form = container.find("form");
                  var path = json.path;
                  var action_path;
                  var ga_name;
                  ...
                  phoneConfirmationType = (json === null || json === void 0 || (_json$properties = json.properties) === null || _json$properties === void 0 ? void 0 : _json$properties.confirmationType) || ubeSession$1('phoneConfirmationType');
                  ...
                  if (/-reg$/.test(path)) {
                    ...
                    action_path = "/esb/" + path + "/submission";
                    if (phoneConfirmationType) ubeSession$1('phoneConfirmationType', phoneConfirmationType);
                    if (isMBEPForm && !isP1Domain()) {
                      gaPushObject({
                        event: 'dl-pageview',
                        pageURL: window.location.href,
                  ...
                  function ajaxSubmitFormInternal(data, isSearchOnly) {
                    ...
                    if (options.data.skipEmailOptin === 'false') {
                    ...
                    $__default["default"].ajax({
                      ...
                      function submissionSuccess(result) {
                        ...
                        if (name.includes('phone-send')) {
                          ...
                          ubeSession$1('phoneConfirmationType', null);
                          ubeSession$1('preferredMethod', null);
                  ...
                  function initializeFaceCapture() {
                    var _options$config2, _options$config3;
                    var renderContainer = container.find(".ube-camera-container");
                    var renderTarget = container.find(".ube-camera-render");
                    var captureButton = container.find(".ube-camera-capture");
                    var fallbackTarget = container.find(".ube-camera-fallback");
                    var vendorsList = (_options$config2 = options.config) === null || _options$config2 === void 0 || (_options$config2 = _options$config2.av) === null || _options$config2 === void 0 || (_options$config2 = _options$config2.face) === null || _options$config2 === void 0 ? void 0 : _options$config2.vendorsList;
                    var faceState = {
                      ...
                      tryCount: 1,
                      options: options,
                      priorityFaceVendor: ((_options$config3 = options.config) === null || _options$config3 === void 0 || (_options$config3 = _options$config3.av) === null || _options$config3 === void 0 || (_options$config3 = _options$config3.face) === null || _options$config3 === void 0 ? void 0 : _options$config3.primaryFaceVendor) || 'idx',
                      onSuccess: onSuccess,
                      onError: onError
                    };
                  ...
                  fieldNames.forEach(function (key) {
                    ...
                    if (/-reg$/.test(path) && autoAddSessionFields.indexOf(key) > -1 && ubeSession$1(key)) {
                      if (!initialValue || initialValue === "") initialValue = ubeSession$1(key);
                    }
                  ...
                  container.off("click", ".ube-trigger-event").on("click", ".ube-trigger-event", function (e) {
                    ...
                    if (event == "resendCode" || event == "resendPhoneCode" || event == "resendEmailCode") {
                      var _category16 = isQRForm ? 'dl_qr_' : 'dl_auth_';
    ...
    $__default["default"](document).ready(function () {
      function checkUtm() {
        var query = getJsonFromUrl();
        if (query.utm_content) ubeSession$1("utmContent", query.utm_content);
        if (query.utm_campaign) ubeSession$1("utmCampaign", query.utm_campaign);
        if (query.utm_source) ubeSession$1("utmSource", query.utm_source);
        if (query.utm_medium) ubeSession$1("utmMedium", query.utm_medium);
      }
