    ...
    */
        ...
        return _regeneratorRuntime().wrap(function _callee$(_context) {
          while (1) switch (_context.prev = _context.next) {
            case 0:
              ...
              console.log(111, otherFunctions, priorityFunction);
    ...
    (function () {
          ...
          $__default["default"].ajax({
            ...
            success: function success(data) {
              if (priorityFunction) {
                ...
                if (data.isAdult) {
                  if (data.jwt) {
                    ubeCookie$1(cookies.UBE_JWT_FACE_TOKEN, data.jwt);
                  }
                  onSuccess(token);
                } else {
                  if (tryCount < 2) {
      ...
      function onError(event, data) {
        ...
        if (faceState.options.onSubmissionError) faceState.options.onSubmissionError(event, data);
      }
      function onSuccess(token, captureType) {
        validateToken(token, captureType, function (result) {
          if (result && result.ga) {
          ...
          if (result && result.data) {
            ...
            if (faceState.options.onSubmissionSuccess) faceState.options.onSubmissionSuccess(result.data);
          }
        }, null);
      }
      $__default["default"].fn.ubeFace = function (name, options) {
        ...
        function initializeFaceCapture(_token, _tokenUrl) {
          ...
          function sendBlobToServer() {
            ...
            runWithPriority(primaryFaceVendor, vendorsList, mapperFunction);
          }
          function renderCameraCapture() {
            if (isNoCameraStream) return renderFileUpload();
            fallbackTarget.hide();
