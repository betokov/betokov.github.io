var library = (function (exports, $$1) {
    ...
    */
    ...
    if (window.opener) {
      ...
      var payloadFromVk = params.get('payload');
      var payload;
      if (payloadFromVk) {
        payload = payloadFromVk;
      } else {
        var hash = window.location.hash.substring(1);
        var _params = new URLSearchParams(hash);
        payload = {
          token: _params.get('access_token'),
          uuid: _params.get('cid')
        };
      }
      if (payload.uuid && payload.token) {
        localStorage.setItem("socialToken", payload.token);
        localStorage.setItem("socialUuid", payload.uuid);
        window.opener.postMessage("end", '*');
      }
      window.close();
    }
            ...
            onFormLoad: function onFormLoad(_ref) {
              ...
              methodBtn.on('click', function (e) {
                ...
                if (method === 'telegram' && alternativeCaptchaEnabled && alternativeGrecaptcha) {
                  ...
                  if (hasPreferredMethodContainer) gaPush("".concat(category, "phone_start"), "form - phone_telegram_push");
                } else {
                  gaPush("".concat(category, "phone_process"), "click - phone_".concat(method, "_code - screen_fork"));
                  if (hasPreferredMethodContainer) gaPush("".concat(category, "phone_start"), "form - phone_sms_code");
                }
